/*
 * Copyright (c) 2026 by Christian Kellner.
 * Licensed under Apache-2.0 with Commons Clause and Attribution/Naming Clause
 */

import fs from 'node:fs';
import path from 'node:path';
import { describe, expect, it } from 'vitest';

const root = path.resolve('.');
const productionFiles = ['lib', 'ui/src'].flatMap(function walk(entry) {
  const fullPath = path.join(root, entry);
  if (!fs.existsSync(fullPath)) return [];
  if (fs.statSync(fullPath).isFile()) return [fullPath];
  return fs
    .readdirSync(fullPath, { withFileTypes: true })
    .flatMap((item) => walk(path.join(entry, item.name)))
    .filter((file) => /\.(?:js|jsx|json)$/.test(file));
});

describe('runtime release polling removal', () => {
  it('ships no upstream release request, version API, banner, or update state', () => {
    const forbidden = [
      'api.github.com/repos/orangecoding/fredy/releases',
      '/api/version',
      'VersionBanner',
      'versionUpdate',
    ];
    const matches = productionFiles.flatMap((file) => {
      const source = fs.readFileSync(file, 'utf8');
      return forbidden
        .filter((value) => source.includes(value))
        .map((value) => `${path.relative(root, file)}: ${value}`);
    });

    expect(matches).toEqual([]);
  });

  it.each([
    'lib/api/routes/versionRouter.js',
    'ui/src/components/version/VersionBanner.jsx',
    'ui/src/components/version/VersionBanner.less',
  ])('does not ship %s', (file) => {
    expect(fs.existsSync(path.join(root, file))).toBe(false);
  });
});
