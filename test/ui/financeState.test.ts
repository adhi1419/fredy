...
import type {
  ...
  FinanceStateSetter,
  FinanceTransport,
} from '../../ui/src/services/state/financeState.js';

function setup() {
  const state: FinanceRootState = { finance: createFinanceState() };
  const get = vi.fn<FinanceTransport['get']>();
  const post = vi.fn<FinanceTransport['post']>();
  const set: FinanceStateSetter = (updater) => Object.assign(state, updater(state));
...
describe('finance state domain', () => {
  ...
  it('maps affordability results and clears loading after success', async () => {
    ...
    let resolveRequest!: (response: FinanceResponse) => void;
    post.mockReturnValueOnce(
      new Promise<FinanceResponse>((resolve) => {
