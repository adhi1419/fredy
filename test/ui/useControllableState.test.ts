 ...
 */
const hookState = vi.hoisted(() => ({ cells: [] as unknown[], cursor: 0 }));

vi.mock('react', () => ({
  useState: <T>(initialValue: T): [T, (next: T) => void] => {
    const index = hookState.cursor++;
    if (!(index in hookState.cells)) hookState.cells[index] = initialValue;
    return [
      hookState.cells[index] as T,
      (next: T) => {
 ...
 * @param {*} defaultValue
 */
function render<T>(controlledValue: T | undefined, defaultValue: T): [T, (next: T) => void] {
  hookState.cursor = 0;
