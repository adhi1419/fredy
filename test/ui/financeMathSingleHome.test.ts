 ...
 */
const MATH_MODULES: readonly string[] = [
  'affordability',
  'amortization',
  'calculate',
  'financing',
...
const CONSTANT_COPIES: ReadonlyArray<readonly [string, string]> = [
  ['lib/services/finance/constants.js', 'ui/src/services/finance/constants.ts'],
];
...
describe('finance math has one home', () => {
  it.each(MATH_MODULES)('ui/src holds no copy of %s', (moduleName) => {
    expect(fs.existsSync(path.join('ui/src/services/finance', `${moduleName}.js`))).toBe(false);
    expect(fs.existsSync(path.join('ui/src/services/finance', `${moduleName}.ts`))).toBe(false);
  });

  it('no file under ui/src imports a finance math module', () => {
    const offenders = sourceFiles('ui/src').filter((file) => {
      const source = fs.readFileSync(file, 'utf8');
      return MATH_MODULES.some((moduleName) => source.includes(`finance/${moduleName}.js`));
    });
  ...
  it('the frontend constants copy carries no functions, only values', () => {
    const source = read('ui/src/services/finance/constants.ts');
  ...
  it.each(CONSTANT_COPIES)('%s and %s agree on every shared constant', (backendPath, frontendPath) => {
    ...
    const values = (source: string): Record<string, string> => {
      const found: Record<string, string> = {};
      for (const match of source.matchAll(/^export const ([A-Z_]+) = ([\s\S]*?);$/gm)) {
        const name = match[1];
        const value = match[2];
        if (name != null && value != null) found[name] = value.replace(/\s+/g, '');
      }
      return found;
    };
