...
interface ProviderCountriesTestState {
  provider?: ProviderEntry[];
  jobsData: { jobs?: JobEntry[] };
}

const store = vi.hoisted(() => ({
  state: { provider: [], jobsData: { jobs: [] } } as ProviderCountriesTestState,
}));

vi.mock('react', () => ({ useMemo: <T>(fn: () => T): T => fn() }));
vi.mock('../../ui/src/services/state/store.js', () => ({
  useSelector: <T>(selector: (state: ProviderCountriesTestState) => T): T => selector(store.state),
}));
