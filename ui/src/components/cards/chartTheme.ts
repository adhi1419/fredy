...
import type { Chart, ChartArea, ChartType, Plugin } from 'chart.js';
 ...
 * @returns {void}
 */
export function registerFinanceCharts(): void {
  if (registered) {
    return;
  }
  ChartJS.register(
 ...
 * @returns {string}
 */
export function formatEuro(value: number | string | null | undefined, locale: string = 'de-DE'): string {
 ...
 * @returns {string}
 */
export function formatEuroCompact(value: number | string | null | undefined, locale: string = 'de-DE'): string {
  if (value == null || !Number.isFinite(Number(value))) {
    return '';
  }
...
/** Options for the custom `zinsbindungBand` plugin. */
export interface ZinsbindungBandOptions {
  endYear?: number;
  label?: string;
}

/** Options for the custom `ruleMarker` plugin. */
export interface RuleMarkerOptions {
  value?: number;
  label?: string;
}

/** Options for the custom `centerTotal` plugin. */
export interface CenterTotalOptions {
  value?: string;
  label?: string;
}

// Register the three custom plugin option keys with chart.js so charts can set them in `plugins`
// without a cast. Each is optional, matching chart.js's own plugin option typing.
declare module 'chart.js' {
  interface PluginOptionsByType<TType extends ChartType> {
    zinsbindungBand?: ZinsbindungBandOptions;
    ruleMarker?: RuleMarkerOptions;
    centerTotal?: CenterTotalOptions;
  }
}

/**
 * The shared chart.js option surface the finance charts build on. It is intentionally a plain
 * structural type rather than `ChartOptions<T>`: each chart spreads it and then narrows the result
 * to its own chart type, which is where the concrete `ChartOptions<'bar'|'line'|...>` check lands.
 */
export interface BaseChartOptions {
  ...
  scales: {
    x: {
      ...
      border: { color: string };
      ticks: { color: string; font: { family: string; size: number }; maxRotation: number; autoSkipPadding: number };
    };
    y: {
      grid: { color: string; drawBorder: boolean };
...
export function baseChartOptions({
  locale = 'de-DE',
  legend = false,
}: { locale?: string; legend?: boolean } = {}): BaseChartOptions {
  return {
    responsive: true,
 ...
 */
export const zinsbindungBandPlugin: Plugin<'line', ZinsbindungBandOptions> = {
  id: 'zinsbindungBand',
  beforeDatasetsDraw(chart: Chart, _args: unknown, options: ZinsbindungBandOptions) {
