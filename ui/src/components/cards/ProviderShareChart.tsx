...
/** The two shapes `data` can arrive in. */
type ProviderShareData = { labels?: string[]; values?: number[] } | Array<{ type?: string; value?: number }> | null;
 ...
 */
function toRows(data: ProviderShareData): Array<{ label: string; value: number | undefined }> {
...
interface ProviderShareChartProps {
  ...
  /** Used to phrase the single-provider case concretely. */
 ...
 */
export default function ProviderShareChart({ data = [], totalListings = 0 }: ProviderShareChartProps) {
        ...
        tooltip: {
          ...
          callbacks: {
            label: (ctx: TooltipItem<'bar'>) => {
              ...
              return `${ctx.dataset.label}: ${x < 1 ? t('dashboard.providerBelowOnePercent') : `${x} %`}`;
            },
          },
        },
      },
