...
interface OperatorCoverage {
  [technology: string]: boolean | undefined;
}

interface MobileConnectivity {
  neutral?: Record<string, boolean | undefined>;
  operators?: Record<string, OperatorCoverage>;
...
interface Connectivity {
  ...
  sharePercent?: number | null;
  source?: string;
  technologies?: Record<string, TechnologyCoverage | undefined>;
  mobile?: MobileConnectivity | null;
}
 ...
 */
export default function ConnectivityCard({ connectivity }: ConnectivityCardProps): ReactElement {
  ...
  const speed = formatDownstream(connectivity.maxDownMbit ?? null);
  const share = roundShare(connectivity.sharePercent ?? null);
  const attribution = connectivity.source == null ? undefined : SOURCE_ATTRIBUTION[connectivity.source];
