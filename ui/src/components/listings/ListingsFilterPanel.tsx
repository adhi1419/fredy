...
interface ListingsFilterPanelProps {
  ...
  /** Applies a URL patch. */
  onChange: (patch: ListingFilterPatch) => void;
  jobs?: ConfiguredJobMetadata[];
  providers?: NamedFilterOption[];
  availableProviders?: string[] | null;
  financeComplete: boolean;
  affordabilityHelp: string;
...
export default function ListingsFilterPanel({
...
}: ListingsFilterPanelProps): ReactElement {
  ...
  // Semi's Select onChange types its value as a broad union (string | number | object | array).
  // These are all single-value controls whose options carry only string or number values, so a
  // cleared control is null and a set one is coerced back to the scalar the URL patch expects.
  const asString = (val: unknown): string | null => (val == null ? null : String(val));
  const asNumber = (val: unknown): number | null => (val == null ? null : Number(val));
    ...
    >
      ...
      <FilterGroup title={t('listings.filterGroupApplication')}>
        <FilterSelect
          ...
          showClear
          onChange={(val) => onChange({ status: asString(val), page: 1 })}
          value={values.status}
          style={{ width: '100%' }}
        >
        ...
        <FilterGroup title={t('listings.filterGroupFit')}>
          ...
          {financeComplete && (
            <FilterSelect
              ...
              showClear
              onChange={(val) => {
                onChange({ afford: asString(val), page: 1 });
              }}
              value={values.afford}
              style={{ width: '100%' }}
            >
          ...
          {hasAddresses && (
            <FilterSelect
              ...
              showClear
              onChange={(val) => onChange({ commute: asString(val), page: 1 })}
      ...
      {connectivityEnabled && (
        <FilterGroup title={t('listings.filterGroupConnectivity')}>
          <FilterSelect
            ...
            showClear
            onChange={(val) => {
              onChange({ down: asNumber(val), page: 1 });
            }}
            value={values.down}
            style={{ width: '100%' }}
          >
          ...
          */}
          ...
          <FilterSelect
            ...
            onChange={(val) => {
              ...
              // drawer shows.
              onChange({ mtech: asString(val), mop: val == null ? null : values.mop, page: 1 });
            }}
            value={values.mtech}
            style={{ width: '100%' }}
          >
          ...
          */}
          <FilterSelect
            ...
            onChange={(val) => onChange({ mop: asString(val), page: 1 })}
            value={values.mop}
            style={{ width: '100%' }}
          >
      ...
      <FilterGroup title={t('listings.filterGroupSource')}>
        <FilterSelect
          ...
          showClear
          onChange={(val) => onChange({ provider: asString(val), page: 1 })}
        ...
        <FilterSelect
          ...
          showClear
          onChange={(val) => onChange({ job: asString(val), page: 1 })}
          value={values.job}
          style={{ width: '100%' }}
        >
          {jobs?.map((job) => (
