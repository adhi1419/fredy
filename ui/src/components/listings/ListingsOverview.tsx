...
import type { ListingFilterValues } from '../../services/listings/listingFilters.js';
import type { NamedFilterOption } from '../../services/listings/listingFilters.js';
import ListingsGrid from '../grid/listings/ListingsGrid.jsx';
import type { ListingItem } from '../grid/listings/ListingsGrid.jsx';
...
interface TravelTimeQuery {
  travelTimeMode: string;
  travelTimeMaxMinutes: number;
}

/** A listings page as the store slice holds it. */
interface ListingsDataSlice {
  result?: ListingItem[];
  totalNumber?: number;
  availableProviders?: string[] | null;
}

/** The one-shot per-user deletion preference. */
interface ListingDeletionPreference {
  skipPrompt?: boolean;
  hardDelete?: boolean;
}

/** The user settings this page reads. */
interface OverviewUserSettings {
  listings_view_mode?: 'grid' | 'table';
  listing_deletion_preference?: ListingDeletionPreference | null;
  home_addresses?: unknown;
  [key: string]: unknown;
}

/** The store slices the listings overview reads. */
interface ListingsOverviewState {
  listingsData?: ListingsDataSlice | null;
  provider?: NamedFilterOption[];
  jobsData: { jobs: NamedFilterOption[] };
  userSettings: { settings?: OverviewUserSettings | null };
  generalSettings: { settings?: { connectivityEnabled?: boolean } | null };
}

/** The store actions the listings overview dispatches. */
interface ListingsOverviewActions {
  listingsData: {
    getListingsData: (query: Record<string, unknown>) => void;
    toggleListingWatch: (id: string) => Promise<void>;
    setListingStatus: (id: string, status: string | null) => Promise<void>;
    restoreListings: (ids: string[]) => Promise<void>;
    reactivateListings: (ids: string[]) => Promise<void>;
  };
  userSettings: {
    setListingsViewMode: (mode: string) => Promise<void>;
 ...
 *
 * @param value e.g. `transit:30`.
 */
function toTravelTimeQuery(value: string | null): TravelTimeQuery | null {
...
const ListingsOverview = (): ReactElement => {
  const t = useTranslation();
  const locale = useLocale();
  const listingsData = useSelector((state: ListingsOverviewState) => state.listingsData);
  const providers = useSelector((state: ListingsOverviewState) => state.provider) ?? [];
  const jobs = useSelector((state: ListingsOverviewState) => state.jobsData.jobs);
  const userSettings = useSelector((state: ListingsOverviewState) => state.userSettings.settings);
  const generalSettings = useSelector((state: ListingsOverviewState) => state.generalSettings.settings);
  const actions = useActions<ListingsOverviewActions>();
  ...
  const filterValues = values as unknown as ListingFilterValues;
  const {
    page,
    sort: sortField,
    dir: sortDir,
  ...
  const setPage = (value: number) => setValue('page', value);
  const setSortField = (value: string) => setValue('sort', value);
  const setSortDir = (value: string) => setValue('dir', value);
  ...
  const handleWatch = async (e: MouseEvent, item: ListingItem) => {
    e.preventDefault();
    e.stopPropagation();
    try {
  ...
  const handleStatusChange = async (item: ListingItem, nextStatus: string | null) => {
    try {
  ...
  const handleDelete = (id: string) => {
    if (listingDeletionPref?.skipPrompt) {
      confirmDeletion(listingDeletionPref.hardDelete ?? false, false, id);
      return;
    }
  ...
  const switchViewMode = (mode: string) => {
    actions.userSettings.setListingsViewMode(mode).catch((error: unknown) => {
      Toast.error(errorMessage(error, t('common.settingSaveError')));
    });
  };

  const confirmDeletion = async (hardDelete: boolean, remember?: boolean, id: string | null = listingToDelete) => {
    try {
      if (remember) {
  ...
  const listings: ListingItem[] = listingsData?.result || [];
    ...
    <div className="listingsOverview">
      <div className="listingsOverview__topbar">
        ...
        <FilterSelect
          ...
          style={{ width: 220 }}
          value={sortField}
          onChange={(val) => setSortField(String(val))}
        >
      ...
      <ActiveFilterChips
        chips={describeActiveFilters(filterValues, { t, jobs, providers })}
