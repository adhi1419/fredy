...
/** The provider shape the picker sorts on: an id, a display name, and the countries it serves. */
interface ProviderOrderInput {
  id?: string;
  name?: string;
  countries?: string[];
}
 ...
 *
 * @returns The alpha-2 code to group by.
 */
export function groupCountryOf(provider: ProviderOrderInput): string {
 ...
 */
export function compareProviders(a: ProviderOrderInput, b: ProviderOrderInput): number {
 ...
 */
export function sortProviders<T extends ProviderOrderInput>(providers?: readonly T[]): T[] {
