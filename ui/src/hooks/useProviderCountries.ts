...
/** One entry in the store's `provider` slice: an id and the countries it declares. */
interface ProviderSliceEntry {
  id?: string;
  countries?: string[];
}

/** A configured provider on a job: only the id matters for country resolution. */
interface JobProviderEntry {
  id?: string;
}

/** A job as far as country resolution cares: the providers configured on it. */
interface JobWithProviders {
  provider?: JobProviderEntry[];
}

/** The slices of the store these hooks read. */
interface ProviderCountriesState {
  provider?: ProviderSliceEntry[];
  jobsData: { jobs?: JobWithProviders[] };
}

/**
 * Fold a set of provider ids into the countries they serve between them.
 *
 * @param providers - The `provider` store slice.
 * @param providerIds - Ids to resolve.
 * @returns Sorted alpha-2 codes, falling back to the default when nothing resolves.
 */
function countriesOf(providers: ProviderSliceEntry[] | undefined, providerIds: Iterable<string>): string[] {
 ...
 *
 * @param providerData - The job's configured providers, `{id, url}` each.
 * @returns Alpha-2 codes.
 */
export function useCountriesForProviders(providerData?: JobProviderEntry[]): string[] {
  const providers = useSelector((state: ProviderCountriesState) => state.provider);
  const ids = (providerData ?? []).map((entry) => entry?.id).filter((id): id is string => typeof id === 'string');
...
export function useProviderCountries(): string[] {
  const providers = useSelector((state: ProviderCountriesState) => state.provider);
  const jobs = useSelector((state: ProviderCountriesState) => state.jobsData.jobs);
