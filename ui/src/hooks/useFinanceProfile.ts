...
import type { FinanceProfile, RentThresholds } from '../types/finance.js';

/** Ceilings a completed buy profile produces. */
interface BuyThresholds {
  affordableMaxPrice: number;
  stretchMaxPrice: number;
  purchasePriceThreshold: number;
}

/** The `/api/finance/profile-summary` payload, as far as this hook reads it. */
interface FinanceProfileSummary {
  isComplete?: boolean;
  rentComplete?: boolean;
  anyComplete?: boolean;
  profile?: FinanceProfile | null;
  budget?: Record<string, unknown> | null;
  thresholds?: { buy: BuyThresholds | null; rent: RentThresholds | null };
}

/** The store slices this hook reads. */
interface FinanceProfileState {
  finance: { summary: FinanceProfileSummary | null };
  userSettings: { settings: { finance_profile?: FinanceProfile | null } };
}

/** The derived finance profile view every conditional finance surface reads. */
export interface DerivedFinanceProfile {
  isComplete: boolean;
  rentComplete: boolean;
  anyComplete: boolean;
  loaded: boolean;
  stored: FinanceProfile | null;
  profile: FinanceProfile | null;
  budget: Record<string, unknown> | null;
  thresholds: { buy: BuyThresholds | null; rent: RentThresholds | null };
}
 ...
 * chip per row.
 */
export function useFinanceProfile(): DerivedFinanceProfile {
  const summary = useSelector((state: FinanceProfileState) => state.finance.summary);
  const stored = useSelector((state: FinanceProfileState) => state.userSettings.settings.finance_profile) ?? null;
