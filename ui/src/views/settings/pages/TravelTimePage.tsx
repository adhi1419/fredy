...
import type { HomeAddress, UserSettingsEffects } from '../../../services/state/userSettingsState.js';
import { useTranslation } from '../../../services/i18n/i18n.jsx';

/** A departure moment: the time of day a transit journey is measured from. */
interface Departure {
  time: string;
}
...
interface TravelTimeState {
  userSettings: { settings: { home_addresses?: readonly HomeAddress[] } };
}

/** The effects this page dispatches. */
interface TravelTimeActions {
 ...
 */
const DEFAULT_DEPARTURE: Departure = { time: '08:00' };
...
function toRow(a: HomeAddress): AddressRow {
  return {
    ...
    departure: (a.departure as Departure | undefined) || DEFAULT_DEPARTURE,
    mode: a.mode || 'transit',
  };
}

/** Read the `home_addresses` array from an unknown save response. */
function homeAddressesOf(responseJson: unknown): HomeAddress[] {
  if (responseJson != null && typeof responseJson === 'object' && 'home_addresses' in responseJson) {
    const value = (responseJson as { home_addresses?: unknown }).home_addresses;
    if (Array.isArray(value)) return value as HomeAddress[];
  }
  return [];
}
 ...
 */
export default function TravelTimePage(): ReactElement {
  const t = useTranslation();
  const actions = useActions<TravelTimeActions>();

  const homeAddresses = useSelector<TravelTimeState, readonly HomeAddress[] | undefined>(
    (state) => state.userSettings.settings.home_addresses,
  );
  ...
  useEffect(() => {
    ...
    setAddresses(initial.map(toRow));
  }, [homeAddresses]);
        ...
        xhrGet(`/api/user/settings/autocomplete?q=${encodeURIComponent(value)}`)
          .then((response) => {
            if (response.status === 200) {
              setDataSource(response.json as string[]);
            }
          })
          .catch(() => {});
      }, 300),
  ...
  const searchAddress = (value: string, idx: number) => {
  ...
  const handleSave = async () => {
    try {
      ...
      setAddresses(homeAddressesOf(responseJson).map(toRow));
            ...
            <div key={idx} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1 }}>
                <AutoComplete
                  ...
                  showClear
                  onChange={(v) =>
                    setAddresses((prev) => prev.map((r, i) => (i === idx ? { ...r, address: String(v) } : r)))
                  }
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <Select
                    ...
                    onChange={(v) =>
                      setAddresses((prev) => prev.map((r, i) => (i === idx ? { ...r, mode: String(v) } : r)))
                    }
                  >
                    {MODES.map((mode) => (
                      ...
                      <Select
                        ...
                        onChange={(v) =>
                          setAddresses((prev) =>
                            prev.map((r, i) =>
                              i === idx
                                ? { ...r, departure: { ...(r.departure ?? DEFAULT_DEPARTURE), time: String(v) } }
                                : r,
                            ),
                          )
                        }
