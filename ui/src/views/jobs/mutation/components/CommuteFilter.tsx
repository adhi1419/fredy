...
import type { SavedAddress } from '../../../../services/jobs/commuteFilter.js';
import './commuteFilter.less';

/** The commute filter as this control reads and emits it. */
interface CommuteFilterValue {
  action?: string;
...
interface CommuteFilterProps {
  ...
  onChange: (next: { action: string; limits: Record<string, number> } | null) => void;
}
...
/** The store slice this control reads. */
interface CommuteFilterState {
  userSettings: { settings: { home_addresses?: unknown } };
}
 ...
 */
export default function CommuteFilter({ value, onChange }: CommuteFilterProps): ReactElement {
  const t = useTranslation();
  const userSettings = useSelector<CommuteFilterState, { home_addresses?: unknown }>(
    (state) => state.userSettings.settings,
  );
  const addresses = savedAddresses(userSettings) as CommuteAddress[];
  ...
  const action =
    value?.action != null && COMMUTE_ACTIONS.includes(value.action) ? value.action : DEFAULT_COMMUTE_ACTION;
  ...
  const emit = (nextLimits: Record<string, number>, nextAction: string): void => {
    ...
    <div className="commuteFilter">
      ...
      <div className="commuteFilter__action">
        ...
        <Select
          ...
          value={action}
          onChange={(v) => emit(limits, String(v))}
        >
