 ...
 */
export default function ConnectivityPage(): ReactElement {
  const { t, form, setField, connectivityDirty, savingConnectivity, saveConnectivity } =
    useOutletContext<AdminSettingsContext>();

  const setSource = (id: string, enabled: boolean) => {
    ...
    <div className="settingsShell__page">
      <SegmentPart name={t('settings.connectivity')} helpText={t('settings.connectivityHelp')}>
        <Checkbox
          ...
          onChange={(e) => setField('connectivityEnabled', e.target.checked === true)}
        >
            ...
            {CONNECTIVITY_SOURCES.map((id) => (
              <div key={id}>
                <Checkbox
                  ...
                  onChange={(e) => setSource(id, e.target.checked === true)}
                >
