...
import type { RestorePrecheck } from '../../../services/backupRestoreClient';
...
export default function BackupPage(): ReactElement {
  ...
  const [precheckInfo, setPrecheckInfo] = React.useState<RestorePrecheck | null>(null);
  const [restoreBusy, setRestoreBusy] = React.useState(false);
  const [selectedRestoreFile, setSelectedRestoreFile] = React.useState<File | null>(null);
  ...
  const performRestore = React.useCallback(
    async (force: boolean) => {
      ...
      try {
        ...
        console.error(e);
        Toast.error((e instanceof Error && e.message) || t('settings.backupRestoreError'));
      } finally {
