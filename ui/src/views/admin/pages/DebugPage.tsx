...
import type { DebugStatus } from '../../../services/debugLoggingClient';
import { useActions } from '../../../services/state/store';
import { useTranslation } from '../../../services/i18n/i18n.jsx';

const { Text } = Typography;

/** The effects the debug page dispatches. */
interface DebugPageActions {
  generalSettings: { getGeneralSettings: () => Promise<void> };
}
...
export default function DebugPage(): ReactElement {
  ...
  const actions = useActions<DebugPageActions>();

  const [debugStatus, setDebugStatus] = React.useState<DebugStatus | null>(null);
  ...
  const applyDebugStatus = React.useCallback((fresh: DebugStatus) => {
  ...
  const performEnableDebug = React.useCallback(
    async ({ clearPrevious }: { clearPrevious: boolean }) => {
      setDebugBusy(true);
      try {
        const fresh = (await apiEnableDebugLogging({ clearPrevious })) as DebugStatus;
        applyDebugStatus(fresh);
  ...
  const handleToggleDebugLogging = React.useCallback(async () => {
    ...
    if (debugStatus.enabled) {
      setDebugBusy(true);
      try {
        const fresh = (await apiDisableDebugLogging()) as DebugStatus;
        applyDebugStatus(fresh);
        await actions.generalSettings.getGeneralSettings();
  ...
  const handleDownloadDebugBundle = React.useCallback(async () => {
    try {
      ...
      Toast.error(code === 'NO_LOGS' ? t('settings.debugToastNoLogs') : t('settings.debugToastDownloadError'));
    }
  }, [t]);
  ...
  const performClearDebugLogs = React.useCallback(async () => {
    setDebugBusy(true);
    try {
      const fresh = (await apiClearDebugLogs()) as DebugStatus;
      applyDebugStatus(fresh);
