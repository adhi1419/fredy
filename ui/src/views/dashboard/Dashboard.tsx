...
/** Translator used across the dashboard's copy. */
type Translator = (key: string, params?: Record<string, string | number>) => string;

/** The dashboard payload the store loads, as far as this view reads it. */
interface DashboardData {
  ...
  pie?: { labels?: string[]; values?: number[] } | Array<{ type?: string; value?: number }>;
  general?: { lastRun?: number | null; nextRun?: number | null; interval?: number };
}

/** The store slices the dashboard reads. */
 ...
 */
function relativeTime(timestamp: number | null | undefined, t: Translator, now: number = Date.now()): string | null {
...
export default function Dashboard(): ReactElement {
  ...
  const timeValue = (timestamp: number | null | undefined): ReactNode => {
  ...
  if (dashboard != null && (kpis.totalJobs ?? 0) === 0) {
    return (
