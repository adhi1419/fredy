...
type EditorStatus = 'idle' | 'checking' | 'saving' | 'notFound';
...
interface AddressEditorProps {
  /** Whether the current address was already set by the user. */
  isManual?: boolean;
  onSave: (position: SavedPosition) => Promise<void>;
  /** Hand over to pin dropping with this text. */
 ...
 */
export default function AddressEditor({ isManual = false, onSave, onPickOnMap }: AddressEditorProps): ReactElement {
  ...
  const submit = useCallback(
    async (value: string) => {
      ...
      try {
        ...
        coords = response.json as { lat: number; lng: number };
      } catch {
  ...
  const content = (
    <div className="addressEditor">
      ...
      <AutoComplete
        data={suggestions}
        value={query}
        onChange={(value) => search(value as string)}
        onSearch={search}
        onSelect={(value) => submit(value as string)}
