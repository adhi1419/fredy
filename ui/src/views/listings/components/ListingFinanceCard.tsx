...
import type { Budget, FinanceProfile, FinanceResultView, RentAffordability } from '../../../types/finance.js';

import './ListingFinanceCard.less';

const { Text, Title } = Typography;

/** The `GET /api/finance/listing/:id` payload, as far as the card reads it. */
interface ListingFinancePayload {
  result?: FinanceResultView | null;
  scored?: RentAffordability | null;
  budget?: Budget;
  profile?: { personA?: { age?: number | null } | null } | null;
}

/** The listing whose monthly cost this card describes. */
...
interface ListingFinanceCardProps {
  listing: FinanceListing;
}

/** Store actions the finance card dispatches. */
interface ListingFinanceActions {
  finance: { getListingFinance: (id: string) => Promise<ListingFinancePayload | null> };
}

/** A single fact row: label, value, whether it is emphasised, and an optional note. */
type FinanceFact = [label: string, value: string, emphasis: boolean, note?: string];
 ...
 */
export default function ListingFinanceCard({ listing }: ListingFinanceCardProps): ReactElement | null {
  const t = useTranslation();
  const locale = useLocale();
  const navigate = useNavigate();
  const actions = useActions<ListingFinanceActions>();
  const [finance, setFinance] = React.useState<ListingFinancePayload | null>(null);
  ...
  if (isRental) {
    return finance?.scored == null || finance.budget == null ? null : (
 ...
 */
function RentCard({ scored, budget, t, locale }: RentCardProps): ReactElement {
  const navigate = useNavigate();
