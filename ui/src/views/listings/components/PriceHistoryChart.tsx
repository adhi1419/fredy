...
export default function PriceHistoryChart({
  ...
  locale = 'de-DE',
  height = 120,
}: PriceHistoryChartProps): React.ReactElement | null {
  const rows = React.useMemo<ReadonlyArray<PriceHistoryPoint>>(() => (Array.isArray(data) ? data : []), [data]);
      ...
      scales: {
        ...
        y: {
          ...
          ticks: {
            ...
            maxTicksLimit: 4,
            callback: (value) => formatEuro(Number(value), locale),
          },
        },
      },
      plugins: {
        ...
        tooltip: {
          displayColors: false,
          callbacks: {
            title: (items: TooltipItem<'line'>[]) => new Date(Number(items[0]?.label)).toLocaleDateString(locale),
            label: (ctx: TooltipItem<'line'>) => formatEuro(ctx.parsed.y, locale),
          },
        },
      },
