...
/** A home address as `getAddresses` returns it. */
...
/** A listing as the map store slice holds it. */
interface MapListingRecord {
  id: string;
  price?: number | null;
...
/** The store slices the map reads. */
interface MapState {
  listingsData: { mapListings: MapListingRecord[] };
  userSettings: {
    settings?: {
...
/** The store actions the map dispatches. */
...
/** Patch emitted by the map's own controls. */
interface MapControlsPatch {
  style?: string;
...
export default function MapView(): ReactElement {
  ...
  const homeMarkers = useRef<Marker[]>([]);
  // Every React tree mounted into a popup, so they can be torn down with their markers.
  const popupRoots = useRef<Array<() => void>>([]);
  const actions = useActions<MapActions>();
  const navigate = useNavigate();
  const sp = useSearchParams();
  const [searchParams, setSearchParams] = sp;
  const listings = useSelector((state: MapState) => state.listingsData.mapListings);
  const userSettings = useSelector((state: MapState) => state.userSettings.settings);
  const homeAddresses = useMemo<HomeAddress[]>(() => getAddresses(userSettings), [userSettings]);
  ...
  const jobId = urlState.job as string | null;
  const distanceFilter = urlState.distance as number;
  const commuteFilter = urlState.commute as string | null;
  const style: MapStyleName = urlState.style === 'SATELLITE' ? 'SATELLITE' : 'STANDARD';
  const show3dBuildings = urlState.buildings as boolean;
  const showTransit = urlState.transit as boolean;
  const setJobId = (value: string | null) => setUrlValue('job', value);
  const setDistanceFilter = (value: number) => setUrlValue('distance', value);
  const setCommuteFilter = (value: string | null) => setUrlValue('commute', value);
  ...
  const [priceRange, setPriceRange] = useState<[number, number]>([urlPriceMin ?? 0, urlPriceMax ?? 0]);
  ...
  const filterListings = (): MapListingRecord[] => {
   ...
   */
  function withinCommute(listing: MapListingRecord): boolean {
    const parsed = parseCommuteFilter(commuteFilter);
    if (parsed == null) {
      return true;
    }
    return (Array.isArray(listing.travelTimes) ? listing.travelTimes : []).some((entry) => {
      const minutes = entry?.[parsed.mode]?.minutes;
      return minutes != null && minutes <= parsed.maxMinutes;
    });
  }

  useEffect(() => {
    window.deleteListing = (id: string) => deleteListingRef.current(id);
   ...
   */
  const handleControlsChange = (patch: MapControlsPatch) => {
    setValues({
  ...
  useEffect(() => {
    ...
    if (homeAddresses.length > 0) {
      if (distanceFilter > 0) {
        ...
        const cornerBounds = getBoundsFromCoords(corners);
        if (cornerBounds) {
          map.current.fitBounds(cornerBounds, {
            padding: 20,
            maxZoom: 15,
            duration: 0,
          });
              ...
              <div className="map-panel">
                <div className="map-panel__row">
                  ...
                  <Select
                    ...
                    showClear
                    size="small"
                    onChange={(val) => setJobId((val as string) ?? null)}
                    value={jobId}
                    style={{ width: 160 }}
                  >
                    {jobs?.map((j) => (
                ...
                <div className="map-panel__row">
                  ...
                  <Select
                    ...
                    size="small"
                    onChange={(val) => setDistanceFilter(val as number)}
                ...
                {homeAddresses.length > 0 && (
                  <div className="map-panel__row">
                    ...
                    <Select
                      ...
                      showClear
                      size="small"
                      onChange={(val) => setCommuteFilter((val as string) ?? null)}
                      value={commuteFilter}
                      style={{ width: 150 }}
                    >
