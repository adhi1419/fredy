...
import type { BudgetView, FinanceProfile, FinanceResultView, RentThresholds } from '../../types/finance.js';
...
/** Which half of the profile a save/delete acts on. */
type Section = 'rent' | 'buy';

/** The POST /api/finance/calculate payload for the current draft. */
interface CalculateSummary {
  result?: FinanceResultView | null;
  isComplete?: boolean;
  rentComplete?: boolean;
  budget?: BudgetView | null;
  thresholds?: { rent?: RentThresholds | null } | null;
}

/** The store actions the calculator invokes. */
interface FinanceCalculatorActions {
  finance: { calculate: (profile: FinanceProfile) => Promise<CalculateSummary | null> };
  userSettings: {
    saveFinanceSection: (input: { section: Section; profile: FinanceProfile }) => Promise<void>;
    deleteFinanceSection: (section: Section) => Promise<void>;
  };
}
...
interface SectionActionsProps {
  section: 'rent' | 'buy';
  /** Whether this tab's inputs are complete enough to save. */
  canSave: boolean;
  /** Whether this tab is currently persisted. */
  saved: boolean;
  saving: boolean;
  deleting: boolean;
  onSave: () => void;
...
function SectionActions({ section, canSave, saved, saving, deleting, onSave, onDelete, hint, t }: SectionActionsProps) {
  return (
...
export default function FinanceCalculator() {
  ...
  const actions = useActions<FinanceCalculatorActions>();
  ...
  // soon as it arrives. The cast reflects that deliberate partial: the runtime object is exactly
  // EMPTY_DRAFT, the forms fill the rest in as the user types.
  const [draft, setDraft] = React.useState<FinanceProfile>(
    () => storedProfile ?? (EMPTY_DRAFT as unknown as FinanceProfile),
  );
  ...
  const [draftSummary, setDraftSummary] = React.useState<CalculateSummary | null>(null);
  ...
  const patchProfile = (patch: Partial<FinanceProfile>) => {
    edited.current = true;
    setDraft((current) => ({ ...current, ...patch }));
  };
  const patchFinancing = (patch: Partial<FinanceProfile['financing']>) => {
    edited.current = true;
    setDraft((current) => ({ ...current, financing: { ...current.financing, ...patch } }));
  };
  const patchRenting = (patch: Partial<NonNullable<FinanceProfile['renting']>>) => {
    edited.current = true;
    setDraft((current) => ({
      ...current,
      renting: { nebenkostenPct: current.renting?.nebenkostenPct ?? null, ...patch },
    }));
  };
   ...
   */
  const autoSaveSection = async (section: Section) => {
  ...
  const [defaultTab] = React.useState<Section>(() => (searchParams.get('dealType') === 'buy' ? 'buy' : 'rent'));
  ...
  return (
    <div className="finance">
      ...
      <Tabs
        ...
        activeKey={activeTab}
        onChange={(value) => {
          if (value === 'rent' || value === 'buy') setActiveTab(value);
        }}
        keepDOM={false}
