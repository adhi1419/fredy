...
import type { RateScenario } from '../../../types/finance.js';
...
interface ScenarioFormProps {
  ...
  /** The same scenarios as the server calculated them, used to fill in the field the user left blank. */
  computed?: Array<{ monthlyPayment: number; tilgung: number }>;
  onChange: (scenarios: RateScenario[]) => void;
}
 ...
 */
export default function ScenarioForm({ scenarios, computed = [], onChange }: ScenarioFormProps) {
  const t = useTranslation();

  const update = (index: number, patch: Partial<RateScenario>) =>
  ...
  // again.
  const setTilgung = (index: number, tilgung: number | null) =>
    update(index, { tilgung: tilgung ?? 0, monthlyPayment: null });
  ...
  const setPayment = (index: number, monthlyPayment: number | null) =>
        ...
        <div className="scenarioRow" key={scenario.id ?? index}>
          ...
          <div className="financeForm__grid scenarioRow__fields">
            ...
            <NumberField
              ...
              onChange={(fixedYears) => update(index, { fixedYears: fixedYears ?? 0 })}
            />
            <NumberField
            ...
            <NumberField
              ...
              onChange={(sondertilgungPerYear) => update(index, { sondertilgungPerYear: sondertilgungPerYear ?? 0 })}
            />
          </div>
          {scenarios.length > 1 && (
