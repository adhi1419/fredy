...
import {
  ...
  type CenterTotalOptions,
} from '../../../components/cards/chartTheme.js';
import { useTranslation, useLocale } from '../../../services/i18n/i18n.jsx';
import type { FinancingResultView } from '../../../types/finance.js';
 ...
 */
const centerTotalPlugin: Plugin<'doughnut', CenterTotalOptions> = {
  id: 'centerTotal',
  afterDatasetsDraw(chart: Chart, _args: unknown, options: CenterTotalOptions) {
...
interface CostBreakdownChartProps {
  /** Result of calculateFinancing. */
  financing?: FinancingResultView | null;
  height?: number;
}
 ...
 */
export default function CostBreakdownChart({ financing, height = 280 }: CostBreakdownChartProps) {
  const t = useTranslation();
  const locale = useLocale();

  const { labels, values, total } = React.useMemo(() => {
    if (financing == null || financing.purchasePrice <= 0) {
      return { labels: [] as string[], values: [] as number[], total: 0 };
    }
    ...
    ].filter(([, value]) => (value as number) > 0) as Array<[string, number]>;
        ...
        tooltip: {
          ...
          callbacks: {
            label: (item: TooltipItem<'doughnut'>) => {
              ...
              const share = total > 0 ? ((parsed / total) * 100).toFixed(1) : '0';
              return `${item.label}: ${formatEuro(parsed, locale)} (${share} %)`;
            },
          },
        },
      },
