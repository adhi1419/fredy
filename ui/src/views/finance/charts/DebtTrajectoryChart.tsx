...
interface DebtTrajectoryChartProps {
  /** Scenarios from calculateFinancing. */
  scenarios?: ComputedScenario[];
  /** Age today, used to annotate the tooltip. */
 ...
 */
export default function DebtTrajectoryChart({
  scenarios = [],
  currentAge = null,
  height = 320,
  ...
  const { labels, datasets, fixedYears } = React.useMemo(() => {
    const usable = scenarios.filter((scenario) => (scenario?.schedule?.months?.length ?? 0) > 0);
    if (usable.length === 0) {
      return { labels: [] as string[], datasets: [], fixedYears: 0 };
    }
    ...
    const sets = usable.map((scenario, index) => {
      ...
      return {
        ...
        backgroundColor: isPrimary
          ? (context: ScriptableContext<'line'>) => makeAreaGradient(context.chart.ctx, context.chart.chartArea, color)
          : withAlpha(color, 0),
      };
    });
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: (item: TooltipItem<'line'>) => `${item.dataset.label}: ${formatEuro(item.parsed.y, locale)}`,
          },
        },
      },
      scales: {
