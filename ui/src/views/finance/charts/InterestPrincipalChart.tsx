 ...
 */
export default function InterestPrincipalChart({ scenario, height = 280 }: InterestPrincipalChartProps) {
    ...
    return {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            title: (items: TooltipItem<'bar'>[]) => t('finance.chart.yearLabel', { year: String(items[0]?.label) }),
            label: (item: TooltipItem<'bar'>) => `${item.dataset.label}: ${formatEuro(item.parsed.y, locale)}`,
          },
        },
      },
      scales: {
