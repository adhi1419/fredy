...
import {
  ...
  withAlpha,
  type RuleMarkerOptions,
} from '../../../components/cards/chartTheme.js';
import { useTranslation, useLocale } from '../../../services/i18n/i18n.jsx';
import type { BudgetView } from '../../../types/finance.js';
 ...
 */
const ruleMarkerPlugin: Plugin<'bar', RuleMarkerOptions> = {
  id: 'ruleMarker',
  afterDatasetsDraw(chart: Chart, _args: unknown, options: RuleMarkerOptions) {
...
interface BudgetChartProps {
  ...
  /** The housing cost being considered. */
  monthlyRate?: number;
  /**
   * What to call that cost in the legend and tooltip. Defaults to the mortgage wording; the
   * renting side passes the warm rent instead, because "Baufinanzierung" on a rental chart
   * describes something the user is not doing.
   */
  rateLabel?: string;
  height?: number;
}
 ...
 */
export default function BudgetChart({ budget, monthlyRate = 0, rateLabel, height = 150 }: BudgetChartProps) {
  ...
  ].filter(([, value]) => (value as number) > 0) as Array<[string, number, string]>;
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: (item: TooltipItem<'bar'>) => {
            const share = (((item.parsed.x ?? 0) / budget.netIncome) * 100).toFixed(0);
