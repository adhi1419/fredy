...
class FredyPipelineExecutioner {
  ...
  async _notify(newListings) {
    ...
    return Promise.all(sendNotifications).then(async () => {
      // A completed notification is durable evidence the user was told about this listing exactly
      // once. Persisting it write-once is what lets a later reconcile pass tell "already notified"
      // from "notification never finished" without ever resending. A failure here must not cost the
      // notification that just went out, so it is logged and swallowed.
      const notifiedAt = Date.now();
      await Promise.all(
        newListings.map((listing) =>
          markNotificationComplete(listing.id, notifiedAt).catch((err) =>
            logger.warn(`Could not persist notification-complete for listing ${listing.id}`, err?.message ?? err),
          ),
        ),
      );
      return newListings;
