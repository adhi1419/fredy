...
import { setInquiryMessage } from './services/storage/listingsStorage.js';
import { generateInquiryMessage, isMessageGeneratorEnabled } from './services/messageGenerator.js';
import { deliverInquiry } from './services/inquiries/deliverInquiry.js';
import { isInquiryProfileReady } from './services/inquiries/profile.js';
import { inquiryRequiresMessage, supportsInquirySending } from './services/inquiries/sendInquiry.js';
import { resolveAutomaticInquiryPolicy } from './services/providers/applicationPolicyExecution.js';
import { sendToUser } from './services/sse/sse-broker.js';
import * as notify from './notification/notify.js';
import { orchestrateNotificationDelivery } from './notification/notificationDeliveryOrchestrator.js';
import { reserveDelivery, finishDelivery } from './services/storage/notificationLedgerStorage.js';
...
import { planReconcile } from './services/jobs/searchRepair.js';
...
class FredyPipelineExecutioner {
   ...
   * @param {{maxDetailFetches?: number|null, providerSource?: Object, applicationCapability?: Object}} [options]
   *   Run options. `providerSource` is the exact source currently iterated; `applicationCapability`
   *   is its loaded provider metadata. `maxDetailFetches` caps detail enrichment in tests; null means
   *   no cap, which is what production wants.
   */
  constructor(providerConfig, job, providerId, similarityCache, browser, options = {}) {
    /** @type {ProviderConfig} */
    this._providerConfig = providerConfig;
    /** @type {Object|null} */
    this._providerSource = options?.providerSource ?? null;
    /** @type {boolean} */
    this._legacyAutoSendInquiry = job.autoSendInquiry === true;
    this._applicationCapability = options?.applicationCapability;
    ...
    /** @type {'rent'|'buy'|null} */
    this._jobDealType = job.dealType ?? null;
    /** @type {string} */
  ...
  execute() {
    return (
      Promise.resolve(urlModifier(this._providerConfig.url, this._providerConfig.sortByDateParam))
        ...
        .then(this._generateInquiryMessages.bind(this))
        .then(this._sendInquiryMessages.bind(this))
  ...
  /**
   * Reconcile the KNOWN listings of this provider/job and repair their eligible state — the pass
   * `execute()` cannot do because `_findNew` drops everything already stored.
   *
   * It is deliberately separate from `execute()`: a fresh scrape short-circuits with
   * `NoNewListingsWarning` the moment nothing is new, which is exactly the run where the stored
   * rows most need a look. Both the scheduled/manual run and the "Run & repair" API route call it.
   *
   * The guarantee: the external world sees at most ONE new side effect per eligible listing
   * (a single reserved inquiry send), and a second reconcile over the rows the first one repaired
   * is a no-op — every field it fills, it fills once.
   *
   * @param {{ now?: number, settleMs?: number }} [options]
   * @returns {Promise<ReturnType<typeof planReconcile>['summary']>}
   */
  async reconcile() {
    const rows = await getKnownListingsForRepair(this._jobKey, this._providerId);
    if (rows.length === 0) {
      return planReconcile([]).summary;
    }

    const job = await getJob(this._jobKey);
    const userId = job?.userId;
    const settings = userId ? await getUserSettings(userId) : null;
    const profile = settings?.inquiry_profile ?? null;
    const user = userId ? await getUser(userId) : null;

    // Field repair is always local. An external inquiry is considered only when the real applicant
    // profile is ready; the existing per-source/capability/listing policy is re-applied below for
    // every row before deliverInquiry can reserve anything.
    const autoActEligible =
      this._jobDealType === 'rent' &&
      profile != null &&
      typeof profile === 'object' &&
      isInquiryProfileReady(profile, user?.username, this._providerId);

    const { plans, summary } = planReconcile(rows, { autoActEligible });
    const rowsById = new Map(rows.map((row) => [row.id, row]));
    const countries = summary.coordinatesRepaired > 0 ? await getCountriesForProvider(this._providerId) : null;

    let coordinatesRepaired = 0;
    let inquiryTextRepaired = 0;
    let notificationCompleteRepaired = 0;
    let missedActions = 0;
    ...
    for (const plan of plans) {
      ...
      if (plan.repairs.coordinates && row.address) {
        try {
          const coords = await geocodeAddress(row.address, countries);
          if (coords && coords.lat !== -1 && coords.lng !== -1) {
            coordinatesRepaired += await repairListingCoordinates(row.id, { lat: coords.lat, lng: coords.lng });
          }
        } catch (err) {
          logger.warn(`Repair: could not geocode listing ${row.id}`, err?.message ?? err);
        }
      }

      let message = typeof row.inquiry_message === 'string' ? row.inquiry_message : '';
      if (plan.repairs.inquiryText && isMessageGeneratorEnabled() && profile != null) {
        try {
          message = await generateInquiryMessage(profile, row);
          inquiryTextRepaired += await setInquiryMessage(row.id, message);
        } catch (err) {
          logger.warn(`Repair: could not draft inquiry for listing ${row.id}`, err?.message ?? err);
        }
      }

      if (plan.repairs.notificationComplete) {
        try {
          notificationCompleteRepaired += await markNotificationComplete(row.id, row.notified_at ?? row.notifiedAt);
        } catch (err) {
          logger.warn(`Repair: could not reconcile notification state for ${row.id}`, err?.message ?? err);
        }
      }

      if (plan.missedAction?.kind === 'inquiry') {
        const policyAllowsInquiry = resolveAutomaticInquiryPolicy({
          source: this._providerSource,
          legacyAutoSendInquiry: this._legacyAutoSendInquiry,
          providerId: this._providerId,
          capability: this._applicationCapability,
          listing: row,
        });
        if (!policyAllowsInquiry || !supportsInquirySending(this._providerId, row)) continue;
        if (inquiryRequiresMessage(this._providerId, row) && message.trim() === '') {
          // No draft to send with, and drafting is handled above only on future runs — skip rather
          // than send an empty inquiry.
          continue;
        }
        try {
          // deliverInquiry reserves through reserveInquirySend: the durable, contract-proven barrier
          // that makes a concurrent or repeated pass a no-op. A never-attempted/failed row is the
          // only one that reserves; `unknown`/`sent`/`sending` were already excluded by the planner.
          ...
          if (result.started) missedActions += 1;
        } catch (err) {
          logger.warn(`Repair: could not retry inquiry for listing ${row.id}`, err?.message ?? err);
        }
      }
    }
  ...
  async _fetchDetails(newListings) {
    ...
    const userId = (await getJob(this._jobKey))?.userId;
    const enabledProviders = (await getUserSettings(userId))?.provider_details ?? [];
  ...
  async _filterByArea(newListings) {
    ...
    if (toDeleteListingByIds.length > 0) {
      await deleteListingsById(toDeleteListingByIds);
    }
   ...
   * @throws {NoNewListingsWarning} When every listing is filtered out.
   */
  async _filterAfterDetails(listings) {
    if (typeof this._providerConfig.filter !== 'function') {
      return listings;
    }
    const userId = (await getJob(this._jobKey))?.userId;
    const enabled = (await getUserSettings(userId))?.blacklist_filter_on_provider_details === true;
    if (!enabled) {
      return listings;
    }
  ...
  async _findNew(listings) {
    ...
    const knownHashes = new Set((await getKnownListingHashesForJobAndProvider(this._jobKey, this._providerId)) || []);
   ...
   */
  /**
   * Eagerly draft a rental inquiry message for each listing about to be notified.
   *
   * Runs on only the listings that survived every filter — the exact set the user is about to be
   * told about — so no LLM call is ever spent on a flat that was filtered out. Gated twice: the
   * feature must be enabled (GEMINI_API_KEY set) and the job owner must have an inquiry profile,
   * otherwise this is a no-op and the run behaves exactly as before the feature existed.
   *
   * Failure here must never cost the notification (same rule as travel times): a generation error
   * is logged and the listing goes out with just its link and image. The text is both persisted
   * (for the detail view) and attached to the in-memory listing (so the notification adapters can
   * include it in this same run without a re-fetch).
   *
   * @param {ParsedListing[]} newListings
   * @returns {Promise<ParsedListing[]>}
   * @private
   */
  async _generateInquiryMessages(newListings) {
    if (newListings.length === 0 || !isMessageGeneratorEnabled()) {
      return newListings;
    }
    const userId = (await getJob(this._jobKey))?.userId;
    ...
    const profile = (await getUserSettings(userId))?.inquiry_profile;
    if (profile == null || typeof profile !== 'object' || Object.keys(profile).length === 0) {
      // No profile means every draft would be generic filler — skip rather than spend quota.
      return newListings;
    }
    for (const listing of newListings) {
      try {
        const message = await generateInquiryMessage(profile, listing);
        listing.inquiryMessage = message;
        await setInquiryMessage(listing.id, message);
      } catch (err) {
        // A rate-limited call is retried with backoff inside the generator; reaching here means it
        // still failed after those retries (or failed for another reason). Now that ImmoScout is
        // paginated a first run can surface dozens of new listings, so a single failure must not
        // cost the rest of the batch: log it and move on. The listing still goes out with its link.
        logger.warn(`Could not draft an inquiry message for listing ${listing.id}`, err?.message ?? err);
      }
    }
    return newListings;
  }
  ...
  /**
   * Submit generated drafts for a job that explicitly enabled automatic inquiry delivery.
   *
   * Only supported rental providers enter this path. Every listing is independently guarded by a
   * storage reservation, and every failure is swallowed after recording its terminal state so a
   * provider outage or incomplete profile can never cost the ordinary listing notification.
   *
   * @param {ParsedListing[]} newListings
   * @returns {Promise<ParsedListing[]>}
   * @private
   */
  async _sendInquiryMessages(newListings) {
    if (this._jobDealType !== 'rent' || !supportsInquirySending(this._providerId) || newListings.length === 0) {
      return newListings;
    }

    const eligibleListings = newListings.filter((listing) =>
    ...
    if (eligibleListings.length === 0) return newListings;

    const userId = (await getJob(this._jobKey))?.userId;
    const [settings, user] = userId ? await Promise.all([getUserSettings(userId), getUser(userId)]) : [null, null];
    const profile = settings?.inquiry_profile;
    if (profile == null || typeof profile !== 'object') {
      logger.warn(`Auto-send skipped for job ${this._jobKey}: applicant profile is missing.`);
      return newListings;
    }
    if (!isInquiryProfileReady(profile, user?.username, this._providerId)) {
      logger.warn(`Auto-send skipped for job ${this._jobKey}: ${this._providerId} profile fields are incomplete.`);
      return newListings;
    }

    for (const listing of eligibleListings) {
      if (!supportsInquirySending(this._providerId, listing)) {
        logger.debug(`Auto-send skipped for listing ${listing.id}: partner delivery is not supported.`);
        continue;
      }
      const message = typeof listing.inquiryMessage === 'string' ? listing.inquiryMessage : '';
      if (inquiryRequiresMessage(this._providerId, listing) && message.trim() === '') {
        logger.warn(`Auto-send skipped for listing ${listing.id}: no generated inquiry draft.`);
        continue;
      }
      try {
        const result = await deliverInquiry({
        ...
        if (!result.started) {
          logger.warn(`Auto-send skipped for listing ${listing.id}: delivery was already reserved.`);
        }
      } catch (error) {
        logger.warn(`Could not auto-send inquiry for listing ${listing.id}: ${error?.message ?? 'unknown error'}`);
      }
    }
    return newListings;
  }
  ...
  async _notify(newListings) {
    ...
    const userId = (await getJob(this._jobKey))?.userId;
    const language = (await getUserSettings(userId))?.language ?? 'en';
    // formatListing spreads the original listing, so the deterministic listing id survives onto the
    // formatted copy — the ledger keys on it and the adapter renders the formatted fields.
    const formattedListings = newListings.map((listing) => formatListing(listing, language));
    const settings = await getSettings();
    const baseUrl = settings?.baseUrl || process.env.FRONTEND_URL || '';

    // Per-listing/per-channel delivery. The orchestrator reserves each (listing, channel) in the
    // ledger before any adapter side effect, sends one listing to one channel at a time (preserving
    // adapter pacing), settles each outcome, and writes the aggregate notification-complete marker
    // only once every intended channel for a listing is confirmed sent. A listing with no
    // configured channels is never marked complete. A failure inside is logged and swallowed by the
    // orchestrator so it never costs the notification that just went out.
    await orchestrateNotificationDelivery(
      {
        listings: formattedListings,
        notificationConfig: this._jobNotificationConfig,
      ...
      },
      {
        reserveDelivery,
        finishDelivery,
        sendOneToChannel: notify.sendOneToChannel,
        markNotificationComplete,
      },
    );
    return newListings;
  ...
  async _save(newListings) {
    ...
    await storeListings(this._jobKey, this._providerId, newListings);
    return newListings;
  }
   ...
   * @returns {ParsedListing[]} The same listings, unchanged.
   */
  async _sendNewListingsToUser(newListings) {
    if (newListings.length > 0) {
      try {
        const job = await getJob(this._jobKey);
        const userId = job?.userId;
   ...
   * @private
   */
  async _calculateDistance(listings) {
    ...
    const addresses = getAddresses(await getUserSettings(userId));
    ...
    for (const listing of listings) {
      if (listing.latitude != null && listing.longitude != null) {
        ...
        await updateListingDistances(listing.id, distances);
  ...
  async _calculateTravelTimes(listings) {
    ...
    try {
      ...
      await attachTravelTimes(listings);
    } catch (err) {
   ...
   * @private
   */
  async _filterByCommuteBudget(listings) {
    if (listings.length === 0) return listings;
    ...
    const addresses = addressesWithBudget(getAddresses(await getUserSettings(userId)), filter);
    if (addresses.length === 0) {
      return listings;
    }
    ...
    if (filter.action === 'exclude') {
      await deleteListingsById(overBudget.map((listing) => listing.id));
    }
  ...
  async _filterBySimilarListings(listings) {
    ...
    if (filteredIds.length > 0) {
      await deleteListingsById(filteredIds);
    }
