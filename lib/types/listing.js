 ...
 * @property {string} [inquiryMessage] Generated inquiry draft attached during a pipeline run.
 * @property {{state: 'new'|'applied'|'viewed'|'archived', source?: string|null, changedAt?: number|null, changedBy?: string|null, appliedAt?: number|null, viewedAt?: number|null}} [lifecycle] Canonical user-visible lifecycle.
 * @property {{attempt: {state: 'none'|'sending'|'confirmed'|'failed'|'unknown', startedAt?: number|null, finishedAt?: number|null, requestId?: string|null, error?: string|null}}} [application] Internal provider delivery evidence; never a competing lifecycle.
 * @property {'sending'|'sent'|'failed'|'unknown'} [inquirySendStatus] Delivery state attached during
 *   a pipeline run; stored API rows expose the equivalent snake_case fields.
