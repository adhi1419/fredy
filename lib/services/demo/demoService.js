...
async function getDemoUser() {
  return (await userStorage.getUsers()).find((user) => user.username === userStorage.DEMO_USERNAME) || null;
}
 ...
 */
async function demoChannelRef(userId) {
  const existing = (await channelStorage.getAllChannels()).find(
    (channel) => channel.userId === userId && channel.adapterId === 'demo',
  );
  const id =
    existing?.id ??
 ...
 * @returns {void}
 */
async function clearDemoUserPreferences(userId) {
  await upsertSettings(Object.fromEntries(DEMO_USER_PREFERENCES.map((name) => [name, null])), userId);
}
...
export async function resetDemoUserPreferences() {
  ...
  await clearDemoUserPreferences(demoUser.id);
...
export async function seedDemoJob(providers) {
  ...
  await jobStorage.upsertJob({
    ...
    notificationAdapter: [await demoChannelRef(demoUser.id)],
...
export async function seedDemoFinanceProfile() {
  ...
  await upsertSettings({ finance_profile: demoFinanceProfile() }, demoUser.id);
...
export async function seedDemoHomeAddress() {
  ...
  await upsertSettings(
    { home_addresses: [{ ...DEMO_HOME_ADDRESS, coords: { ...DEMO_HOME_ADDRESS.coords } }] },
    demoUser.id,
  );
  runGeoCordTask();
...
export async function warnOnDefaultAdminPassword() {
  try {
    const admin = await userStorage.getUserWithSecretsByUsername(userStorage.ADMIN_USERNAME);
...
export async function cleanupDemoData() {
  ...
  const doomed = (await jobStorage.getJobs({ includeDisabled: true })).filter(
    (job) => job.userId === demoUser.id && !isDemoJob(job.id),
  );

  if (isFirestore()) {
    // Firestore has no cross-document transactions exposed at this layer; run sequentially.
    for (const job of doomed) {
      await jobStorage.removeJob(job.id);
    }
    await deleteInactiveListingsByJobId(DEMO_JOB_ID);
    await clearDemoUserPreferences(demoUser.id);
  } else {
    SqliteConnection.withTransaction(() => {
      doomed.forEach((job) => jobStorage.removeJob(job.id));
      deleteInactiveListingsByJobId(DEMO_JOB_ID);
      // clearDemoUserPreferences is now async, but the sqlite upsertSettings is
      // still synchronous under the hood — call the storage function directly.
      upsertSettings(Object.fromEntries(DEMO_USER_PREFERENCES.map((name) => [name, null])), demoUser.id);
    });
  }
