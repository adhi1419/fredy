...
import { sourceForCountries } from './sources.js';
...
async function enrichListing(listing, settings, now) {
  ...
  if (source == null) {
    await updateListingConnectivity(listing.id, null, toColumns(null), now);
    return 'empty';
  }
  ...
  await updateListingConnectivity(listing.id, connectivity, toColumns(connectivity), now);
...
export default async function runConnectivitySweep({ now = Date.now(), limit } = {}) {
  ...
  const listings = await getListingsToEnrichConnectivity({
