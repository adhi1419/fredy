 ...
 * @returns {void}
 */
export async function calculateDistanceForJob(jobId, userId) {
  const addresses = getAddresses(await getUserSettings(userId));
  if (addresses.length === 0) {
    return;
  }

  const listings = await getListingsToCalculateDistance(jobId);
  for (const listing of listings) {
    await updateListingDistances(listing.id, distancesToAddresses(listing.latitude, listing.longitude, addresses));
  }
}
 ...
 * @returns {void}
 */
export async function updateDistancesForAddressChange(userId, addresses) {
  const listings = await getListingsForUserToCalculateDistance(userId);
  for (const listing of listings) {
    await updateListingDistances(listing.id, distancesToAddresses(listing.latitude, listing.longitude, addresses));
  }
  ...
  await markTravelTimesDirty(listings.map((listing) => listing.id));
}
 ...
 * @returns {void}
 */
export async function updateDistancesForListing(listingId, latitude, longitude, ownerUserId) {
  const addresses = getAddresses(await getUserSettings(ownerUserId));
  if (addresses.length === 0) {
    return;
  }

  await updateListingDistances(listingId, distancesToAddresses(latitude, longitude, addresses));
}
