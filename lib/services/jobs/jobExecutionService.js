...
export function initJobExecutionService({ providers, intervalMs }) {
  ...
  bus.on('jobs:status', async ({ jobId, running }) => {
    try {
      const recipients = await resolveRecipients(jobId);
  ...
  /**
   * Resolve the owner and explicitly shared users who should receive SSE updates for a job.
   * Admin status is not an implicit product-data share.
   *
   * @param {string} jobId
   * @returns {string[]} unique userIds
   */
  async function resolveRecipients(jobId) {
    const job = await jobStorage.getJob(jobId);
    if (!job) return [];
    const shared = Array.isArray(job.shared_with_user) ? job.shared_with_user : [];
    const recipients = [job.userId, ...shared].filter(Boolean);
  ...
  async function executeJob(job) {
    ...
    try {
      await jobStorage.updateJobLastRunAt(job.id, Date.now());
    } catch (err) {
    ...
    try {
      ...
      for (const prov of jobProviders) {
        try {
          ...
          const executioner = new FredyPipelineExecutioner(providerConfig, job, prov.id, similarityCache, browser, {
            providerSource: prov,
      ...
      try {
        const { archived } = await archiveStaleListingsForJob(job.id);
        if (archived > 0) {
