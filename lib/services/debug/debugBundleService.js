 ...
 *
 * `getAllDebugLogs` uses the asynchronous Firestore API, so it must be awaited.
 * Without the await, the caller receives a Promise and hits `Promise.map is not a
 * function`, resulting in a 500 response from the production download route.
 *
 * @returns {Promise<string>}
 */
export async function renderLogsTxt() {
  const rows = await getAllDebugLogs();
 ...
 * What triage actually reads stays: the URL scheme, port, path, and parameter names. Values that are not
 * URLs, and URLs carrying no host (`/health`, `mailto:`), are returned untouched.
 *
 * @param {string} value
 * @returns {string}
 */
...
export async function buildDebugBundleZip({ settings = null } = {}) {
  const logsContent = (await renderLogsTxt()) || 'No debug log entries are currently stored.\n';
