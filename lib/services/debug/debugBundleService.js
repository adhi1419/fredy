 ...
 *
 * `getAllDebugLogs` is synchronous on the sqlite backend but async on Firestore,
 * so it must be awaited — awaiting a plain value is a no-op, so this stays correct
 * on both. Without the await, Firestore returned a Promise here and the caller hit
 * `Promise.map is not a function`, 500-ing the /download route in prod.
 *
 * @returns {Promise<string>}
 */
