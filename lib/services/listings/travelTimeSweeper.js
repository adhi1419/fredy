...
function carryOver(row, label) {
  return {
    ...
    // Stored rows expose JSON text while the writer accepts structured values, so
    // parse these fields to keep one shape on both sides.
...
async function updateListing(listing, origins, { now, staleBefore, streetBudget }) {
  if (origins.length === 0) {
    ...
    await saveListingTravelTimes(listing.id, [], now);
    return true;
  }

  const stored = (await getTravelTimesForListings([listing.id])).get(listing.id) ?? [];
  ...
  if (deferred > 0) {
    ...
    // month later.
    if (entries.length > 0) {
      await saveListingTravelTimes(listing.id, entries, now, { stamp: false });
    }
    return false;
  }
  ...
  if (answered === 0 && entries.every((entry) => !hasAnyTime(entry))) {
    ...
    await recordTravelTimeFailure(listing.id, now);
    return false;
  }

  await saveListingTravelTimes(listing.id, entries, now);
  return true;
}
...
export default async function runTravelTimeSweep({ now = Date.now(), limit } = {}) {
  ...
  const due = await getListingsDueForTravelTimes({ limit: perRun, staleBefore: settings.staleBefore });
  if (due.length === 0) {
  ...
  for (const listing of due) {
    if (!originsByUser.has(listing.user_id)) {
      ...
      const addresses = usableAddresses(getAddresses(await getUserSettings(listing.user_id)));
