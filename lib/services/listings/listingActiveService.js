...
export default async function runActiveChecker(opts = {}) {
  ...
  const listings = await getListingsDueForActiveCheck({
    limit,
  ...
  if (failedIds.length > 0) {
    const exhausted = await recordActiveCheckFailures(failedIds);
  ...
  if (listingsSetToInactive.length > 0) {
    ...
    await deactivateListings(listingsSetToInactive);
  } else {
    logger.debug('No listings need to be set inactive.');
  }

  if (probedIds.length > 0) {
    await markListingsChecked(probedIds);
  }
  logger.debug(
