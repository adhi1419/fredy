...
// (imported lazily inside initSimilarityCache — see comment there)
...
export const startSimilarityCacheReloader = () => {
  ...
  setInterval(() => {
    initSimilarityCache().catch((err) => logger.warn('Similarity cache reload failed', err?.message ?? err));
  }, reloadCycle);
};
 ...
 * @returns {void}
 */
export const initSimilarityCache = async () => {
  // Lazy import: the listingsStorage facade top-level-awaits its backend, and a
  // static import here would close an import cycle (listingsStorage evicts
  // through this module) and deadlock ESM linking.
  const { getAllEntriesFromListings } = await import('../storage/listingsStorage.js');
  const allEntries = await getAllEntriesFromListings();
