...
import { normalizeJobProviders } from '../../providers/applicationPolicy.js';
import { deleteDeliveriesForJobId } from './notificationLedgerStorage.js';
...
// Firestore forbids nested arrays (an array whose entries are themselves
// arrays). A spatialFilter is GeoJSON whose Polygon coordinates are
// [[[lng,lat], ...]] — triple-nested — so it cannot be stored natively and
// the write fails with INVALID_ARGUMENT "invalid nested entity". Persist it as
// a JSON string and rehydrate on read. Every other job field (specFilter /
// commuteFilter objects, string arrays, arrays of maps) is Firestore-legal and
// stays native.
const serializeSpatialFilter = (sf) => (sf == null ? null : JSON.stringify(sf));
const parseSpatialFilter = (sf) => {
  ...
  return sf; // defensive: a legacy raw object (pre-fix writes never succeeded)
};
...
async function toApiJob(snap, channelMap) {
  ...
  return {
    ...
    provider: await normalizeJobProviders(
      d.provider ?? [],
      Object.prototype.hasOwnProperty.call(d, 'autoSendInquiry') ? d.autoSendInquiry === true : null,
    ),
    shared_with_user: d.sharedWithUser ?? [],
    notificationAdapter: hydrateNotificationAdapter(d.notificationAdapter, channelMap),
    spatialFilter: parseSpatialFilter(d.spatialFilter),
    specFilter: d.specFilter ?? null,
    commuteFilter: d.commuteFilter ?? null,
    autoSendInquiry: d.autoSendInquiry === true,
...
export const upsertJob = async ({
  ...
  const normalizedProvider = await normalizeJobProviders(
    provider ?? [],
    typeof autoSendInquiry === 'boolean'
      ? autoSendInquiry
      : snap.exists && Object.prototype.hasOwnProperty.call(snap.data(), 'autoSendInquiry')
        ? snap.data().autoSendInquiry === true
        : null,
    { legacyPrecedence: typeof autoSendInquiry === 'boolean' },
  );
  if (snap.exists) {
    ...
    await ref.update({
      ...
      ...(typeof autoSendInquiry === 'boolean' ? { autoSendInquiry } : {}),
    ...
    await ref.set({
      ...
      specFilter: specFilter ?? null,
      commuteFilter: commuteFilter ?? null,
      autoSendInquiry: autoSendInquiry === true,
...
export const getJob = async (jobId) => {
  // Public reads treat a missing/empty id as "not found"; Firestore's
  // .doc('') would otherwise throw.
