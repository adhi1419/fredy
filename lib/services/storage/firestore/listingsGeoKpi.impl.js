 ...
 * revisits. Soft-deleted tombstones are excluded: an out-of-area/similar listing was rejected on
 * purpose and must not be resurrected by repair.
 *
 * @param {string} jobId
 * @param {string} providerId
 * @returns {Promise<Object[]>} API-row shaped listings (snake_case) for the planner.
 */
export const getKnownListingsForRepair = async (jobId, providerId) => {
  ...
  return snapshot.docs
    ...
    .filter(({ d }) => d.manuallyDeleted !== true)
    .map(({ id, d }) => ({
      id,
      hash: d.hash ?? null,
      ...
      notification_complete: d.notificationComplete ?? null,
