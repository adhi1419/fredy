...
import { legacyStatusPayload, lifecycleFromData, transitionLifecycle } from '../../listings/listingLifecycle.js';
import { isListingStaleForArchive } from '../../listings/staleArchive.js';
import { deleteDeliveriesForListingIds } from './notificationLedgerStorage.js';
...
export const purgeExpiredInactiveListings = async ({ retentionDays, now = Date.now() }) => {
  ...
  await deleteDeliveriesForListingIds(purgeList.map((doc) => doc.id));
 ...
 * @param {string} id
 * @param {{status: 'sent'|'failed'|'unknown', requestId?: string|null, sentAt?: number|null, error?: string|null}} result
...
export const finishInquirySend = async (id, { status, requestId = null, sentAt = null, error = null }) => {
  ...
  return FirestoreConnection.getConnection().runTransaction(async (transaction) => {
    ...
    if (status === 'sent') {
      ...
      // Keep old clients compatible while making the canonical lifecycle win.
      update.status = legacyStatusPayload('applied', 'applied', changedAt);
    }
...
/**
 * Set a canonical lifecycle action while retaining the old structured status
 * projection for clients in the compatibility window.
 * @param {string} id
...
export const setListingStatus = (id, status, changedBy = null) => {
  ...
  return (async () => {
    ...
    await ref.update({
      lifecycle,
      status: legacyStatusPayload(lifecycle.state, normalized, changedAt),
    });
    return 1;
  })();
};
...
/**
 * Auto-archive a single job's listings that have aged past one week.
 *
 * This is the storage half of the one-week sweep the job run drives. It is scoped to exactly one
 ...
 * is the pure {@link isListingStaleForArchive} rule (strictly older than one week, never a row that
 * is already archived), so the boundary is tested independently of Firestore.
 *
 * It transitions a stale listing to the canonical `archived` state exactly the way a manual
 * {@link setListingStatus} would, writing both the lifecycle object and the legacy status
 * projection so clients in the compatibility window follow. It NEVER deletes and never touches a
 * soft-deleted (`manuallyDeleted`) row — archiving is a lifecycle move, not a purge, and the
 * retention purge remains the only thing that removes anything. Running it twice is a no-op: the
...
export const archiveStaleListingsForJob = async (jobId, { now = Date.now() } = {}) => {
  ...
  for (const doc of snapshot.docs) {
    const data = doc.data();
    // Never archive a soft-deleted tombstone: it is already out of the user's active view, and the
    // retention purge owns its eventual removal.
    if (data.manuallyDeleted === true) continue;
    const lifecycle = lifecycleFromData(data);
    if (isListingStaleForArchive({ createdAt: data.createdAt, lifecycleState: lifecycle.state }, { now })) {
  ...
  await batched(stale, (batch, { id, lifecycle }) => {
    ...
    batch.update(listingsCol().doc(id), {
      lifecycle: next,
      status: legacyStatusPayload(next.state, 'archived', now),
    });
  });
