...
/**
 * Store (or clear) the eager-generated inquiry draft for a listing. Written by
 * the pipeline right before notification so the Telegram second message and
 * the detail view can read it. Empty normalises to NULL.
 *
 * @param {string} id
 * @param {string|null} message
 * @returns {number} rows affected
 */
export const setInquiryMessage = (id, message) => {
  if (!id) return 0;
  const trimmed = typeof message === 'string' ? message.trim() : null;
  const value = trimmed && trimmed.length > 0 ? trimmed : null;
  const res = SqliteConnection.execute(`UPDATE listings SET inquiry_message = @message WHERE id = @id`, {
    id,
    message: value,
  });
  return res?.changes ?? 0;
...
/**
 * Atomically reserve one listing for inquiry delivery.
 *
 * A failed attempt may be retried manually. `sending`, `sent`, and `unknown` are deliberately
 * terminal for reservation: after a process loss or ambiguous network result, sending again could
 * contact the provider twice.
 *
 * @param {string} id
 * @param {number} [startedAt]
 * @returns {boolean}
 */
export const reserveInquirySend = (id, startedAt = Date.now()) => {
  if (!id) return false;
  const res = SqliteConnection.execute(
    `UPDATE listings
     SET inquiry_send_status = 'sending',
         inquiry_send_started_at = @startedAt,
         inquiry_sent_at = NULL,
         inquiry_request_id = NULL,
         inquiry_send_error = NULL
     WHERE id = @id
       AND (inquiry_send_status IS NULL OR inquiry_send_status = 'failed')`,
    { id, startedAt },
  );
 ...
 */
export const finishInquirySend = (id, { status, requestId = null, sentAt = null, error = null }) => {
  if (!id) return 0;
  ...
  const res = SqliteConnection.execute(
    `UPDATE listings
     SET inquiry_send_status = @status,
         inquiry_sent_at = @sentAt,
         inquiry_request_id = @requestId,
         inquiry_send_error = @error
     WHERE id = @id AND inquiry_send_status = 'sending'`,
    { id, status, sentAt, requestId, error },
  );
  return res?.changes ?? 0;
};
