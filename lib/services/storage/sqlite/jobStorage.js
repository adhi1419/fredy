 ...
 * @param {boolean|null} [params.autoSendInquiry] - Explicit per-job inquiry opt-in. Null preserves
 *   the stored value on update; new jobs default to false.
 * @returns {void}
 */
    ...
    SqliteConnection.execute(
      `UPDATE jobs
       SET enabled = @enabled,
           ...
           auto_send_inquiry = COALESCE(@autoSendInquiry, auto_send_inquiry),
       ...
       WHERE id = @id`,
      {
        ...
        autoSendInquiry: typeof autoSendInquiry === 'boolean' ? (autoSendInquiry ? 1 : 0) : null,
        dealType: dealType ?? null,
      },
    );
  } else {
    SqliteConnection.execute(
      `INSERT INTO jobs (id, user_id, enabled, name, blacklist, provider, notification_adapter, shared_with_user, spatial_filter, spec_filter, commute_filter, auto_send_inquiry, deal_type)
       VALUES (@id, @user_id, @enabled, @name, @blacklist, @provider, @notification_adapter, @shareWithUsers, @spatialFilter, @specFilter, @commuteFilter, @autoSendInquiry, @dealType)`,
      {
        id,
        user_id: ownerId,
        enabled: enabled ? 1 : 0,
        ...
        specFilter: specFilter ? toJson(specFilter) : null,
        commuteFilter: commuteFilter ? toJson(commuteFilter) : null,
        autoSendInquiry: autoSendInquiry === true ? 1 : 0,
...
export const getJob = (jobId) => {
  const row = SqliteConnection.query(
    `SELECT j.id,
            ...
            j.auto_send_inquiry AS autoSendInquiry,
  ...
  return {
    ...row,
    enabled: !!row.enabled,
    autoSendInquiry: !!row.autoSendInquiry,
