 ...
 *
 * `password` (the hash) is left out of the projection entirely rather than nulled afterwards,
 * so there is no way to accidentally ask for it. Callers that genuinely need it use
 * {@link getUserWithSecretsByUsername}.
 *
...
export const ensureDemoUserExists = async () => {
  ...
  if (existing.length === 0) {
    SqliteConnection.execute(
      `INSERT INTO users (id, username, password, last_login, is_admin)
       VALUES (@id, @username, @password, NULL, 0)`,
      {
        id: nanoid(),
        username: DEMO_USERNAME,
...
export const ensureAdminUserExists = async () => {
  ...
  if (!anyUser) {
    SqliteConnection.execute(
      `INSERT INTO users (id, username, password, last_login, is_admin)
       VALUES (@id, @username, @password, @last_login, 1)`,
      {
        id: nanoid(),
