...
function rejectionMessage(phase, response, result, unknown = false) {
  const detail = providerErrorText(result);
  const summary = unknown
    ? `The ImmoScout inquiry outcome is unknown after ${phase}`
    : `ImmoScout rejected the inquiry during ${phase}`;
  return `${summary} (HTTP ${response.status})${detail ? `: ${detail}` : ''}.`;
}
...
export async function sendImmoscoutInquiry({
  ...
  try {
    ...
    const providerError = transportErrorText(error);
    throw new InquiryDeliveryError(
      `Could not read the ImmoScout contact requirements${providerError ? `: ${providerError}` : ''}.`,
      { outcome: 'failed', phase: 'requirements', providerError, cause: error },
    );
  }

  let detailResult;
  try {
    detailResult = await responseJson(detailResponse);
  } catch (error) {
    throw new InquiryDeliveryError(
      `ImmoScout contact requirements response could not be read (HTTP ${detailResponse.status}).`,
      { outcome: 'failed', status: detailResponse.status, phase: 'requirements', cause: error },
    );
  }
  if (!detailResponse.ok) {
    const providerError = providerErrorText(detailResult);
    throw new InquiryDeliveryError(
      `ImmoScout contact requirements request failed (HTTP ${detailResponse.status})${providerError ? `: ${providerError}` : ''}.`,
      {
        outcome: 'failed',
  ...
  const { data } = contactDataOf(detailResult);
  ...
  try {
    validationResult = await responseJson(validationResponse);
  } catch (error) {
    throw new InquiryDeliveryError(
      `ImmoScout validation response could not be read (HTTP ${validationResponse.status}).`,
      { outcome: 'failed', status: validationResponse.status, phase: 'validation', cause: error },
    );
  }
  if (!validationResponse.ok) {
    const providerError = providerErrorText(validationResult);
    throw new InquiryDeliveryError(rejectionMessage('validation', validationResponse, validationResult), {
      outcome: 'failed',
  ...
  if (!sendResponse.ok) {
    ...
    const providerError = providerErrorText(result);
    throw new InquiryDeliveryError(rejectionMessage('real send', sendResponse, result, unknown), {
