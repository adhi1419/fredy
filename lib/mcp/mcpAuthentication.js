 ...
 */
export async function authenticateToolCall(extra) {
 ...
 */
export async function authenticateRequest(req, resource = null) {
  ...
  if (token.startsWith('fredy_')) return await validateMcpToken(token);
  return resource ? await validateAccessToken(token, resource) : null;
}
