...
// Initialize the storage backend (required for standalone usage)
if (isFirestore()) {
  const { default: FirestoreConnection } = await import('../services/storage/firestore/FirestoreConnection.js');
  await FirestoreConnection.init();
} else {
  const { default: SqliteConnection } = await import('../services/storage/SqliteConnection.js');
  await SqliteConnection.init();
  const { runMigrations } = await import('../services/storage/migrations/migrate.js');
  await runMigrations();
}
