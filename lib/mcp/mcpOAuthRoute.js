...
async function authorizationRequest(request) {
  ...
  const client = typeof params?.client_id === 'string' ? await getClient(params.client_id) : null;
...
async function mcpOAuthPlugin(fastify) {
  ...
  fastify.post('/api/oauth/authorize', async (request, reply) => {
    ...
    const user = await getUser(request.session.currentUser);
  ...
  fastify.post('/api/oauth/token', async (request, reply) => {
    ...
    if (
      ...
      tokens = await redeemAuthorizationCode({
        code: body.code,
      ...
      tokens = await refreshAccessToken({ refreshToken: body.refresh_token, clientId: body.client_id });
