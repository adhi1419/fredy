...
export function createMcpServer() {
  ...
  server.tool(
    ...
    async ({ page, pageSize, filter }, extra) => {
      const { user, error } = await authenticateToolCall(extra, 'list_jobs');
  ...
  server.tool(
    ...
    async ({ jobId }, extra) => {
      const { user, error } = await authenticateToolCall(extra, 'get_job');
  ...
  server.tool(
    ...
    async (
      ...
      const { user, error } = await authenticateToolCall(extra, 'list_listings');
  ...
  server.tool(
    ...
    async ({ listingId }, extra) => {
      const { user, error } = await authenticateToolCall(extra, 'get_listing');
      if (error) return normalizeError(error, 'get_listing');

      const listing = await getListingById(listingId, user.id, user.isAdmin);
      if (!listing) {
  ...
  server.tool(
    ...
    async ({ listingId }, extra) => {
      const { user, error } = await authenticateToolCall(extra, 'get_photo_for_listing');
  ...
  server.tool(
    ...
    async (args, extra) => {
      const { user, error } = await authenticateToolCall(extra, 'calculate_financing');
      ...
      const profile = normalizeProfile((await getUserSettings(user.id))?.finance_profile);
      ...
      if (args.listingId) {
        listing = await getListingById(args.listingId, user.id, user.isAdmin);
        if (!listing) {
