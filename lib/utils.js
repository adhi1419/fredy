 ...
 * On the firestore backend the sqlite adapter is excluded — it requires a
 * local disk that diskless deployments (Cloud Run) do not have.
 *
 * @returns {Promise<any[]>}
 */
export function getNotificationAdapters() {
  if (!cachedAdaptersPromise) {
    const exclude = isFirestore() ? ['sqlite.js'] : [];
    cachedAdaptersPromise = loadPluginModules(path.join(getDirName(), 'notification', 'adapter'), exclude);
  }
 ...
 * @param {string[]} [exclude] - Filenames to skip (e.g. ['sqlite.js']).
 * @returns {Promise<any[]>} The loaded modules.
 */
export function loadPluginModules(directory, exclude = []) {
  const excludeSet = new Set(exclude);
  const fileNames = fs
    .readdirSync(directory)
    .filter((fileName) => fileName.endsWith('.js') && !excludeSet.has(fileName))
    .sort();
