...
async function getListings(url) {
  ...
  async function fetchPage(pageUrl) {
    const requestHeaders = { ...headers, ...(searchToken ? { 'X-VON-Search-Token': searchToken } : {}) };
    let response = await fetch(pageUrl, { method: 'GET', headers: requestHeaders });
    if (response.status !== 401) return response;

    const tokenResponse = await fetch(`${metaInformation.baseUrl}api/real-estate/search-token`, {
      method: 'GET',
      headers,
    });
    if (!tokenResponse.ok) return response;
    const tokenBody = await tokenResponse.json();
    searchToken = typeof tokenBody?.token === 'string' ? tokenBody.token : null;
    if (!searchToken) return response;

    response = await fetch(pageUrl, {
      method: 'GET',
      headers: { ...headers, 'X-VON-Search-Token': searchToken },
    });
    return response;
  }
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    const response = await fetchPage(pageUrl.toString());
    if (!response.ok) {
