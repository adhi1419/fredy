...
/**
 * Hard ceiling on pages walked per run. The mobile API caps a page at ~50 listings while a
 * city-wide Berlin search returns well over a thousand (`numberOfPages: 23` in the fixture), so the
 * walk is bounded to keep a first run - or a search someone points at all of Germany - from firing
 * hundreds of requests. New listings only ever appear on the first pages of a sort-by-date search,
 * so the cap costs nothing in steady state and only clips the tail of an unusually large backlog.
 *
 * @type {number}
 */
const MAX_PAGES = 20;
...
/**
 * Map one mobile-API result-list page to the raw listing shape the normalizer expects.
 *
 * @param {any} responseBody Parsed `/search/list` response.
 * @returns {Array<Object>}
 */
function mapResultListItems(responseBody) {
...
/**
 * Fetch a search, walking every result page rather than only the first.
 *
 * The mobile `/search/list` endpoint paginates with a 1-based `pagenumber` and reports the size of
 * the whole result set in every page (`numberOfPages`, `totalResults`). It used to read page one
 * and stop, which silently capped a job at ~50 listings per run - a first run, or any interval with
 * more new listings than fit on one page, dropped the overflow without it ever being filtered.
 * The walk mirrors {@link file://../provider/deutscheWohnen.js Deutsche Wohnen}, which pages the
 * same way with `offset`. `pagenumber` is stripped as website noise before the pipeline sees the
 * URL (see `param-support.js`), so setting it here is safe: it is Fredy's own paging cursor, not a
 * filter the user supplied.
 *
 * @param {string} url Mobile API search URL (already translated from the web URL).
...
async function getListings(url) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    if (!response.ok) {
      ...
      // search that simply found nothing. A later page failing keeps whatever earlier pages
      // returned rather than throwing the whole run away.
      const details = await response.text().catch(() => '');
      logger.error(
        `Error fetching data from ImmoScout Mobile API (page ${page}): ${response.status} ${response.statusText} ${details.slice(0, 500)}`,
      );
      break;
    }

    const responseBody = await response.json();
    const pageRows = mapResultListItems(responseBody);
    rows.push(...pageRows);

    // The response states the size of the whole result set, so the first page already says whether
    // a second exists. Stop when the API reports the last page, when it hands back an empty page, or
    // when it omits the count entirely (a shape change should not spin the loop to the cap).
    const totalPages = Number(responseBody?.numberOfPages);
    if (pageRows.length === 0 || !Number.isFinite(totalPages) || page >= totalPages) {
      break;
    }
  }
