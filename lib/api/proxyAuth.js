 ...
 * or elevated. Everything else (session lifetime and admin checks) is unchanged.
 *
 ...
 */
export async function resolveProxyUser(request, settings) {
  if (settings?.proxyAuthEnabled !== true) return null;
...
export async function proxyAuthHook(request) {
  ...
  const user = await resolveProxyUser(request, settings);
  if (user == null) return;
