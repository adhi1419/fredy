...
/**
 * Jobs the requesting user may see: their own and ones explicitly shared with them.
 *
 * @param {import('fastify').FastifyRequest} request
 * @returns {Object[]}
 */
async function getAccessibleJobs(request) {
  return (await jobStorage.getJobs()).filter((job) => canAccessJob(request.currentUser, job));
}
 ...
 * scope visible to the current user (own + explicitly shared) rather than a process-wide value.
 *
...
export default async function dashboardPlugin(fastify) {
  fastify.get('/', async (request) => {
    const jobs = await getAccessibleJobs(request);
    const settings = await getSettings();

    const totalJobs = jobs.length;
    const totalListings = jobs.reduce((sum, j) => sum + (j.numberOfFoundListings || 0), 0);
    const jobIds = jobs.map((j) => j.id);
    const { numberOfActiveListings, medianPriceOfListings } = await getListingsKpisForJobIds(jobIds);

    const providerPieRaw = await getProviderDistributionForJobIds(jobIds);
    ...
    const perDay = await getListingsPerDayForJobIds(jobIds, TREND_DAYS);
