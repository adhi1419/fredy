...
import { supportsInquirySending } from '../../services/inquiries/sendInquiry.js';
...
export default async function listingsPlugin(fastify) {
  ...
  fastify.post('/:listingId/send-inquiry', async (request, reply) => {
    ...
    if (!supportsInquirySending(listing.provider)) {
      return reply.code(400).send({ message: `Inquiry sending is not supported for ${listing.provider}.` });
    }

    const message = typeof request.body?.message === 'string' ? request.body.message.trim() : '';
    if (!message) return reply.code(400).send({ message: 'The inquiry message is empty.' });
    const profile = (await getUserSettings(userId))?.inquiry_profile ?? {};
