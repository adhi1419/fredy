...
import { canModifyJob } from '../../services/security/access.js';
...
export default async function listingsPlugin(fastify) {
  fastify.get('/table', async (request) => {
    ...
    const allowedStatuses = ['applied', 'rejected', 'accepted', 'viewed', 'viewing', 'archived', 'new', 'none'];
    ...
    if (!nullOrEmpty(jobNameFilter)) {
      const job = await getJob(jobNameFilter);
    ...
    const financeProfile = (await getUserSettings(userId))?.finance_profile ?? null;
    ...
    const pageData = await listingStorage.queryListings({
    ...
    const availableProviders =
      typeof listingStorage.getAvailableProviders === 'function'
        ? await listingStorage.getAvailableProviders({
            jobId: jobIdFilter,
            jobName: jobFilter,
            userId,
            hiddenOnly: normalizedHidden,
  ...
  fastify.get('/map', async (request) => {
    ...
    return await listingStorage.getListingsForMap({
  ...
  fastify.get('/:listingId', async (request, reply) => {
    ...
    const userId = request.currentUser.id;
    const listing = await listingStorage.getListingById(listingId, userId);
    if (!listing) {
    ...
    const thresholds = thresholdsFor((await getUserSettings(userId))?.finance_profile ?? null);
  ...
  fastify.get('/:listingId/priceHistory', async (request, reply) => {
    ...
    if (!(await listingStorage.userCanAccessListing(listingId, userId))) {
      return reply.code(403).send({ message: NO_ACCESS_MESSAGE });
    }
    return await listingStorage.getPriceHistory(listingId);
  });

  // Draft a rental inquiry message for a listing (user reviews + sends it themselves; Fredy never
  // contacts the landlord). On-demand LLM call; 404 when GEMINI_API_KEY is unset.
  fastify.post('/:listingId/draft-message', async (request, reply) => {
    if (!isMessageGeneratorEnabled()) {
      return reply.code(404).send({ message: 'Message drafting is not enabled on this instance' });
    }
    const { listingId } = request.params;
    const userId = request.currentUser.id;
    if (!listingId || !userId) {
    ...
    // The applicant profile is a per-user setting (multi-tenant safe): each user drafts from
    // their own facts. Absent profile still produces a valid generic message.
    ...
    try {
      ...
      logger.error('Failed to draft inquiry message', err);
      return reply.code(502).send({ message: 'Could not generate a message right now. Please try again.' });
    }
  });

  // Send an edited/generated inquiry once. Shared-job viewers and administrators are intentionally
  // not allowed to contact a provider on the owner's behalf; only the owning user's profile may be
  // disclosed. The storage reservation prevents a double-click or concurrent auto-send.
  fastify.post('/:listingId/send-inquiry', async (request, reply) => {
    const { listingId } = request.params;
    const userId = request.currentUser.id;
    if (!listingId || !userId) {
      return reply.code(400).send({ message: 'listingId or user not provided' });
    ...
    const job = await getJob(listing.job_id);
    if (!job || job.userId !== userId) {
      return reply.code(403).send({ message: 'Only the job owner may send an inquiry.' });
    }
    if (!supportsInquirySending(listing.provider, listing)) {
      return reply.code(400).send({ message: `Inquiry sending is not supported for ${listing.provider}.` });
    }

    const message = typeof request.body?.message === 'string' ? request.body.message.trim() : '';
    if (!message && inquiryRequiresMessage(listing.provider, listing)) {
      return reply.code(400).send({ message: 'The inquiry message is empty.' });
    }
    const profile = (await getUserSettings(userId))?.inquiry_profile ?? {};
    ...
    try {
      const result = await deliverInquiry({
        ...
        listing,
        profile,
        accountEmail: request.currentUser?.username,
        message,
      });
      if (!result.started) {
        return reply.code(409).send({
          message: 'This listing already has an inquiry attempt. Check its delivery status before trying again.',
          status: result.status,
        });
      }
      return reply.send(result);
    } catch (error) {
      const isInquiryError = error?.name === 'InquiryDeliveryError';
      if (isInquiryError && error.missingFields.length > 0) {
        return reply.code(422).send({ message: error.message, missingFields: error.missingFields });
      }
      if (isInquiryError && error.outcome === 'unknown') {
        return reply.code(502).send({ message: error.message, status: 'unknown' });
      }
      const statusCode = isInquiryError && error.status >= 400 && error.status < 500 ? 422 : 502;
      logger.warn(`Could not send inquiry for listing ${listingId}: ${error?.message ?? 'unknown error'}`);
      return reply.code(statusCode).send({ message: error?.message ?? 'Could not send inquiry.' });
    }
  });
  ...
  fastify.post('/watch', async (request, reply) => {
    try {
      ...
      await watchListStorage.toggleWatch(listingId, userId);
    } catch (error) {
      logger.error(error);
  ...
  fastify.post('/:listingId/notes', async (request, reply) => {
    ...
    if (!(await listingStorage.userCanModifyListing(listingId, userId))) {
      return reply.code(403).send({ message: NO_ACCESS_MESSAGE });
    }
    try {
      const changes = await listingStorage.setListingNotes(listingId, typeof notes === 'string' ? notes : null);
      if (changes === 0) {
        return reply.code(404).send({ message: 'Listing not found' });
      }
    } catch (error) {
  ...
  fastify.post('/:listingId/address', async (request, reply) => {
    ...
    try {
      ...
      const changes = await listingStorage.setListingAddress(listingId, trimmed, lat, lng);
      if (changes === 0) {
        return reply.code(404).send({ message: 'Listing not found' });
      }
      ...
      const ownerUserId = (await getJob(listing.job_id))?.userId;
      if (ownerUserId) {
      ...
      // second round trip.
      return reply.send(await listingStorage.getListingById(listingId, userId));
    } catch (error) {
      logger.error(error);
  ...
  fastify.post('/:listingId/geocode', async (request, reply) => {
    ...
    try {
      ...
      await listingStorage.updateListingGeocoordinates(listingId, coords.lat, coords.lng);
      const ownerUserId = (await getJob(listing.job_id))?.userId;
      if (ownerUserId) {
      ...
      return reply.send({
        status: 'found',
        listing: await listingStorage.getListingById(listingId, userId),
      });
    } catch (error) {
      logger.error(error);
  ...
  fastify.post('/:listingId/status', async (request, reply) => {
    ...
    // `status` is the legacy payload; `action` is the canonical lifecycle
    // payload. Both use the same storage transition and access checks.
    const requested = action === undefined ? status : action;
    const allowed = [
      'applied',
      'rejected',
      'accepted',
    ...
    const normalized = requested == null ? null : String(requested).toLowerCase();
    if (normalized != null && !allowed.includes(normalized)) {
      return reply.code(400).send({ message: `Invalid status: ${requested}` });
    }
    if (!(await listingStorage.userCanModifyListing(listingId, userId))) {
      return reply.code(403).send({ message: NO_ACCESS_MESSAGE });
    }
    try {
      const changes = await listingStorage.setListingStatus(listingId, normalized, userId);
      if (changes === 0) {
        return reply.code(404).send({ message: 'Listing not found' });
      }
      if (normalized != null && !['new', 'reset', 'restore'].includes(normalized)) {
        await watchListStorage.ensureWatch(listingId, userId);
      }
    } catch (error) {
      logger.error(error);
  ...
  fastify.delete('/job', async (request, reply) => {
    ...
    try {
      ...
      if (!canModifyJob(request.currentUser, job)) {
        return reply
          .code(403)
          .send({ error: 'You are trying to remove listings for a job that is not associated to your user' });
      }
      await listingStorage.deleteListingsByJobId(jobId, hardDelete);
    } catch (error) {
      logger.error(error);
  ...
  fastify.delete('/', async (request, reply) => {
    ...
    try {
      ...
      if (Array.isArray(ids) && ids.length > 0) {
        const allowed = await listingStorage.filterListingIdsForOwner(ids, request.currentUser.id);
        ...
        await listingStorage.deleteListingsById(allowed, hardDelete);
      }
    } catch (error) {
      logger.error(error);
      return reply.code(500).send({ error: error.message });
  ...
  fastify.post('/restore', async (request, reply) => {
    ...
    try {
      ...
      if (Array.isArray(ids) && ids.length > 0) {
        ...
        await listingStorage.restoreListingsById(allowed);
      }
    } catch (error) {
      logger.error(error);
      return reply.code(500).send({ error: error.message });
  ...
  fastify.post('/reactivate', async (request, reply) => {
    ...
    try {
      ...
      if (Array.isArray(ids) && ids.length > 0) {
        ...
        await listingStorage.reactivateListings(allowed);
      }
    } catch (error) {
      logger.error(error);
      return reply.code(500).send({ error: error.message });
