...
export default async function listingsPlugin(fastify) {
  ...
  fastify.get('/:listingId', async (request, reply) => {
    ...
    const userId = request.currentUser.id;
    const listing = await listingStorage.getListingById(listingId, userId, isAdminFn(request));
    if (!listing) {
  ...
  fastify.get('/:listingId/priceHistory', async (request, reply) => {
    ...
    if (!(await listingStorage.userCanAccessListing(listingId, userId, isAdminFn(request)))) {
  ...
  fastify.post('/:listingId/address', async (request, reply) => {
    ...
    try {
      ...
      // second round trip.
      return reply.send(await listingStorage.getListingById(listingId, userId, isAdminFn(request)));
    } catch (error) {
      logger.error(error);
  ...
  fastify.post('/:listingId/geocode', async (request, reply) => {
    ...
    try {
      ...
      return reply.send({
        status: 'found',
        listing: await listingStorage.getListingById(listingId, userId, isAdminFn(request)),
      });
    } catch (error) {
      logger.error(error);
  ...
  fastify.delete('/', async (request, reply) => {
    ...
    try {
      ...
      if (Array.isArray(ids) && ids.length > 0) {
        const allowed = await listingStorage.filterListingIdsForUser(ids, request.currentUser.id, isAdminFn(request));
