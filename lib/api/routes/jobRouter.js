...
import { copyReplyHeadersToRaw } from '../http.js';
import { normalizeJobProviders, ApplicationPolicyValidationError } from '../../services/providers/applicationPolicy.js';
 ...
 * API clients can all send the shape that is natural to them.
...
export default async function jobPlugin(fastify) {
  fastify.get('/', async (request) => {
    return (await jobStorage.getJobs({ includeDisabled: true }))
  ...
  fastify.get('/data', async (request) => {
    ...
    const queryResult = await jobStorage.queryJobs({
  ...
  fastify.get('/events', async (request, reply) => {
    ...
    const raw = reply.raw;
    copyReplyHeadersToRaw(reply, raw);
    reply.hijack();
  ...
  fastify.post('/', async (request, reply) => {
    ...
    try {
      const jobFromDb = await jobStorage.getJob(jobId);
      ...
      for (const channelId of channelRefs) {
        ...
        // `request.currentUser`, not `request.currentUser.id`: the predicate needs the
      ...
      const hasLegacyAutoSendInquiry = typeof autoSendInquiry === 'boolean';
      const normalizedProvider = await normalizeJobProviders(
        provider,
        hasLegacyAutoSendInquiry ? autoSendInquiry : (jobFromDb?.autoSendInquiry ?? null),
        { legacyPrecedence: hasLegacyAutoSendInquiry },
      );
      ...
      await jobStorage.upsertJob({
        ...
        autoSendInquiry: typeof autoSendInquiry === 'boolean' ? autoSendInquiry : null,
      });
    } catch (error) {
      if (error instanceof ApplicationPolicyValidationError || error?.code === 'APPLICATION_POLICY_INVALID') {
  ...
  fastify.delete('/', async (request, reply) => {
    ...
    try {
      ...
      await jobStorage.removeJob(jobId);
    } catch (error) {
      logger.error(error);
      return reply.code(500).send({ error: error.message });
    }
  ...
  fastify.put('/:jobId/status', async (request, reply) => {
    ...
    try {
      ...
      await jobStorage.setJobStatus({ jobId, status });
    } catch (error) {
      logger.error(error);
      return reply.code(500).send({ error: error.message });
    }
