...
export default async function userPlugin(fastify) {
  ...
  fastify.get('/:userId', async (request) => {
    ...
    return await userStorage.getUser(userId);
  });
  ...
  fastify.get('/:userId/mcp-token', async (request, reply) => {
    const { userId } = request.params;
    const mcpToken = await userStorage.getMcpToken(userId);
    if (mcpToken == null) {
  ...
  fastify.delete('/', async (request, reply) => {
    ...
    const allUser = await userStorage.getUsers();
    ...
    await jobStorage.removeJobsByUserId(userId);
