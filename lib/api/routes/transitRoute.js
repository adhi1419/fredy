...
export default async function transitPlugin(fastify) {
  ...
  fastify.get('/travel-times', async (request, reply) => {
    ...
    if (!(await userCanAccessListing(listingId, userId, isAdmin))) {
      return reply.code(403).send({ error: 'Not allowed to access this listing' });
    }

    const listing = await getListingById(listingId, userId, isAdmin);
    if (listing == null) {
    ...
    const ownerId = (await getJob(listing.job_id))?.userId ?? userId;
    const addresses = getAddresses(await getUserSettings(ownerId));
    ...
    try {
      ...
      return {
        travelTimes: (await attachTravelTimes([{ id: listing.id }], { includeGeometry: true }))[0].travelTimes ?? [],
      };
    } catch (error) {
