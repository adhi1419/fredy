...
import { canUseChannel, canEditChannel, canListChannel } from '../../services/security/channelAccess.js';
...
export default async function notificationChannelPlugin(fastify) {
  fastify.get('/', async (request) => {
    const usage = await await channelStorage.getUsageCounts();
    return (await channelStorage.getChannelsVisibleTo(request.currentUser)).map((channel) =>
      toDto(channel, request.currentUser, usage),
    );
  });

  fastify.get('/:id', async (request, reply) => {
    const channel = await channelStorage.getChannel(request.params.id);
    if (channel == null) {
      return reply.code(404).send({ error: 'Notification channel not found' });
    }
    if (!canListChannel(request.currentUser, channel)) {
      return reply.code(403).send({ error: 'You are not allowed to see this notification channel.' });
    }
    // Secrets travel only to the channel's owner. A user who was granted a shared channel - an
    // ordinary user for an `everyone` channel, an admin for an `admin` channel - can open it to
    // clone it, but the credentials come back blank. Admin status alone never reveals a secret it
    // does not own: the "reveal for edit" path is the owner, and a non-owner (admin included)
    // sees only the redacted summary.
    return toDto(channel, request.currentUser, await channelStorage.getUsageCounts(), {
      includeFields: true,
  ...
  fastify.post('/', async (request, reply) => {
    ...
    const existing = id ? await channelStorage.getChannel(id) : null;
    if (id && existing == null) {
      return reply.code(404).send({ error: 'Notification channel not found' });
    }
    ...
    try {
      const channelId = await channelStorage.upsertChannel({
        id: existing?.id,
        userId: existing?.userId ?? request.currentUser.id,
        adapterId: resolvedAdapterId,
        name: name.trim(),
        fields: resolvedFields,
        visibility: resolvedVisibility,
      });
      const saved = await channelStorage.getChannel(channelId);
      return reply.send(toDto(saved, request.currentUser, await channelStorage.getUsageCounts()));
    } catch (error) {
  ...
  fastify.delete('/:id', async (request, reply) => {
    ...
    const jobs = await channelStorage.getJobsUsingChannel(channel.id);
    if (jobs.length > 0) {
      return reply.code(409).send({ error: 'This notification channel is still used by jobs.', jobs });
    }

    await channelStorage.removeChannel(channel.id);
    return reply.send();
  });
  ...
  fastify.post('/:id/try', async (request, reply) => {
    ...
    try {
      ...
      await testFire(adapter, channel.fields, { userId: request.currentUser.id });
      return reply.send();
    } catch (error) {
