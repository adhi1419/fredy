...
export default async function userSettingsPlugin(fastify) {
  ...
  fastify.post('/news-last-seen-version', async (request, reply) => {
    ...
    try {
      await upsertSettings({ news_last_seen_version }, userId);
      return { success: true };
    } catch (error) {
  ...
  fastify.post('/theme', async (request, reply) => {
    ...
    try {
      ...
      const previous = (await getUserSettings(userId)).theme ?? 'dark';
  ...
  fastify.post('/inquiry-profile', async (request, reply) => {
    ...
    try {
      await upsertSettings({ inquiry_profile }, userId);
      return { success: true, inquiry_profile };
    } catch (error) {
