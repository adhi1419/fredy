...
import { isCommonInquiryProfileReady, sanitizeInquiryProfile } from '../../services/inquiries/profile.js';
import {
...
export default async function userSettingsPlugin(fastify) {
  ...
  fastify.post('/inquiry-profile', async (request, reply) => {
    const userId = request.currentUser.id;
    const { inquiry_profile, validate_common = false } = request.body || {};
    ...
    if (validate_common === true && !isCommonInquiryProfileReady(sanitizedProfile)) {
      return reply.code(400).send({ error: 'Common applicant profile fields are incomplete.' });
    }
