...
export default async function loginPlugin(fastify) {
  fastify.get('/user', async (request) => {
    ...
    const currentUser = currentUserId == null ? null : await userStorage.getUser(currentUserId);
    if (currentUser == null) {
      return {};
    }
    return {
  ...
  fastify.post('/', async (request, reply) => {
    // In firebase auth mode the password path does not exist. This is a
    // security boundary, not just UX: firebase-provisioned users carry a
    // hash of the empty string, which the verifier would happily match.
    if (isFirebaseAuth()) {
    ...
    const user = await userStorage.getUserWithSecretsByUsername(username);
    ...
    if (user != null && passwordMatches) {
      ...
      await userStorage.setLastLoginToNow({ userId: user.id });
      ...
      if (hasher.needsRehash(user.password)) {
        try {
          ...
          await userStorage.updatePasswordHash({ userId: user.id, passwordHash: upgraded });
        } catch (err) {
