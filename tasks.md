...
The canonical tracker is current through PR #59 and records the reopened UI convergence plus complete TypeScript/Rust migration goals.
...
- TypeScript: 19 `.ts`/`.tsx` files under `ui/src` (including declaration boundaries and `vite-env.d.ts`) versus 144 `.js`/`.jsx` files; separate strict browser-source and Node-aware frontend-test compiler configurations are active.
...
- [x] Keep each TypeScript conversion behind a real domain/component boundary with executable behavior tests; do not create shallow pass-through seams or one mechanical mega-PR — `listingFilters.ts` shipped in PR #55 and `guidedSearchForm.ts` shipped in PR #58 (`a993bdd`, Pages-only run 35500637948, 39 seconds), both with live Pages/API HTTP 200. The `providerUrl` source/test migration rebase-merged in PR #59 as `366d27e` and deployed Pages-only in 83 seconds via run 35501384403; live Pages/API returned HTTP 200. Luna worker `054dd900` completed the adjacent real `jobValidation` domain/test migration on `feat/job-validation-typescript`; all eight changed files passed parent review and publication validation is pending.
