...
The canonical tracker is current through PR #55 and records the reopened UI convergence plus complete TypeScript/Rust migration goals.
...
- [ ] Slice 1 — replace the legacy `JobGrid`/`JobsTable` Saved Searches index with the approved name/criteria/state/last-run/contextual-action rows; delete obsolete production imports and preserve SSE/run/pause/edit/clone/delete/shared-read-only behavior — Luna implementation deleted the legacy boundaries and migrated the surface/test to TSX; parent added explicit legacy dependency declarations plus separate strict source/test configs, and Luna correction `e3a8f7ec` is converging the heading, direct Run/Resume + overflow actions, and bottom create card.
...
- [ ] Implement Rust Firebase bearer verification, per-request Firestore allowlist enforcement, exact-origin CORS, authenticated SSE, and the frozen HTTP error contract — parent-reviewed dormant `/api/auth/me` with real RS256/X.509, bounded cert/OAuth caches, Firestore REST + emulator contracts, and exact bearer/error/CORS parity is published by PR #56 (`0397b09`) with rebase auto-merge enabled; SSE remains outstanding and Rust still owns 0% production traffic.
