...
The canonical tracker is current through PR #63 and records the reopened UI convergence plus complete TypeScript/Rust migration goals.
...
- [ ] Slice 3 — converge Home List + map framing and decision rows without creating a second map or listing query; preserve marker keyboard access and all Home state across view switches — implementation worker `8c3991f6` completed an uncommitted five-file slice in `fredy-home-list-map-luna` from `00f029c`. Correction worker `ce9d8193` migrated `mapUtils` and its direct grouping test to strict TypeScript, removed the unchecked Home cast and obsolete ambient declaration, and made convergence tests execute the real grouping path. Strict typecheck, 15 focused tests, 597 frontend tests, the true 2,329-test offline suite, 20 foundation tests, lint/format/lock/build, diff hygiene, compliant scratch screenshots, and fresh 11-file AutoCR are green. Independent rendered/new-user re-review remains before publication; storage contracts are not implicated by this frontend-only slice.
- [ ] Slice 4 — replace `ProviderTable` in guided Saved Search with provider-choice cards showing URL, capability, connection/profile readiness, and source-local application policy; keep the four-step controller and payload seam — worker `85ddaf87` produced a substantial partial TSX/card replacement in `fredy-provider-choice-cards-luna` from `00f029c` but hit the 140-turn limit before complete staging, validation, rendered evidence, or review. Resume the same conversation; parent must scrutinize the broad declaration-shim surface and avoid shallow boundaries.
